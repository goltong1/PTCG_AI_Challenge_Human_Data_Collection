...
- 무료 플랜 기본값은 메모리 보호를 위해 전체 활성 게임 워커 1개입니다. 이 값은 사용자 1명이 아니라 **실제 경기 1개**를 뜻하며, PvP에서는 2명이 한 워커를 공유합니다.
...
- 빠른 매칭: 두 사용자가 각각 이름과 60장 덱을 선택해 빠른 매칭을 누릅니다. 첫 번째 사용자의 대기 상태는 워커 슬롯을 사용하지 않으며 두 번째 사용자가 들어와 경기가 만들어질 때만 슬롯 1개를 사용합니다.
...
CABT 서버와 PvP 워커 하나를 실행했을 때 메모리 사용량이 약 350MB 수준이므로, 0.5GB RAM에서는 전체 워커 1개가 안전한 기본값입니다. 이 워커 하나로 PvP 플레이어 2명이 같은 경기에 참가합니다. 큐 대기자와 친구 방 호스트는 워커를 쓰지 않습니다. AI 게임과 PvP 게임을 동시에 실제 실행하려 하면 두 번째 게임만 한도 메시지를 받습니다.
...
- 빠른 매칭 대기자와 친구 방 대기자는 `CABT_MAX_ACTIVE_WORKERS`에 포함되지 않습니다.
- 사용자가 매칭에 들어가면 같은 세션의 이전 AI 게임 또는 리플레이 워커는 자동 종료되어 슬롯을 반환합니다.
- 같은 브라우저의 일반 탭은 동일한 `CABT_SESSION` 쿠키를 공유하므로 서로 대전 상대가 될 수 없습니다. 테스트는 다른 브라우저, 시크릿 창 또는 다른 기기에서 진행하세요.
