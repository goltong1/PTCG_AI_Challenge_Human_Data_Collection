# CABT Web Arena · Railway 배포 가이드

> 이 문서는 Railway를 계속 사용할 경우의 대안 절차입니다. v5의 기본 권장은 도쿄 리전을 선택할 수 있는 Fly.io이며, 자세한 내용은 `DEPLOY_FLYIO.md`를 참고하세요. 온라인 플레이어 매칭은 메모리 기반이므로 Railway에서도 replica를 반드시 1개로 유지해야 합니다.

이 버전은 브라우저에서 여러 사용자가 각각 독립된 CABT 엔진 프로세스로 AI와 대전하고, 플레이어끼리 빠른 매칭·친구 방으로 대전하며, 완료된 결과를 서버의 영구 저장소로 전송하도록 구성되어 있습니다.
