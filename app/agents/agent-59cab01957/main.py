...
_ch_name="_causal_history_"+hashlib.sha1(_HERE.encode()).hexdigest()[:12]
_ch_spec=importlib.util.spec_from_file_location(_ch_name,os.path.join(_HERE,"causal_history.py"))
_ch=importlib.util.module_from_spec(_ch_spec);sys.modules[_ch_name]=_ch;_ch_spec.loader.exec_module(_ch)
CausalHistory=_ch.CausalHistory
_tp_name="_temporal_policy_"+hashlib.sha1(_HERE.encode()).hexdigest()[:12]
_tp_spec=importlib.util.spec_from_file_location(_tp_name,os.path.join(_HERE,"temporal_policy.py"))
_tp=importlib.util.module_from_spec(_tp_spec);sys.modules[_tp_name]=_tp;_tp_spec.loader.exec_module(_tp)
TemporalResidualPolicy=_tp.TemporalResidualPolicy
default_model_path=_tp.default_model_path
_sl_name="_semantic_llm_reasoner_"+hashlib.sha1(_HERE.encode()).hexdigest()[:12]
_sl_spec=importlib.util.spec_from_file_location(_sl_name,os.path.join(_HERE,"semantic_llm_reasoner.py"))
_sl=importlib.util.module_from_spec(_sl_spec);sys.modules[_sl_name]=_sl;_sl_spec.loader.exec_module(_sl)
SemanticLLMReasoner=_sl.SemanticLLMReasoner
default_semantic_llm_config_path=_sl.default_config_path
_cl_name="_crossdeck_league_"+hashlib.sha1(_HERE.encode()).hexdigest()[:12]
_cl_spec=importlib.util.spec_from_file_location(_cl_name,os.path.join(_HERE,"crossdeck_league_policy.py"))
_cl=importlib.util.module_from_spec(_cl_spec);sys.modules[_cl_name]=_cl;_cl_spec.loader.exec_module(_cl)
CrossDeckLeaguePolicy=_cl.CrossDeckLeaguePolicy
_vg_name="_v22_guards_"+hashlib.sha1(_HERE.encode()).hexdigest()[:12]
_vg_spec=importlib.util.spec_from_file_location(_vg_name,os.path.join(_HERE,"v22_deterministic_guards.py"))
_vg=importlib.util.module_from_spec(_vg_spec);sys.modules[_vg_name]=_vg;_vg_spec.loader.exec_module(_vg)
...
_temporal=TemporalResidualPolicy(default_model_path(_HERE))
_deck_ids=[]
try:_deck_ids=[int(x) for x in open(os.path.join(_HERE,"deck.csv"),encoding="utf-8").read().replace(","," ").split()]
except Exception:pass
_semantic_llm=SemanticLLMReasoner(default_semantic_llm_config_path(_HERE),_base.card_table,_base.attack_table,_deck_ids)
_league=CrossDeckLeaguePolicy(_base,_api,os.path.join(_HERE,"crossdeck_league_config.json"))
...
# v22 proven-prefix guard adapter --------------------------------------------
_V22_POFFIN_ID=1086
_V22_STRETCHER_ID=1097
_V22_POKE_PAD_ID=1152
_V22_GRIM_ID=648
_V22_SHADOW_BULLET_ID=937

def _v22_visible_serials(mine,card_id):
 """Return exact own serials visibly outside the deck, including stacks."""
 out=set()
 try:
  for c in list(mine.hand or [])+list(mine.discard or []):
   if int(getattr(c,'id',0) or 0)==card_id:out.add(int(c.serial))
  for p in list(mine.active or [])+list(mine.bench or []):
   if p is None:continue
   if int(getattr(p,'id',0) or 0)==card_id:out.add(int(p.serial))
   for c in list(getattr(p,'preEvolution',[]) or []):
    if int(getattr(c,'id',0) or 0)==card_id:out.add(int(c.serial))
 except Exception:pass
 return out

def _v22_action_ref(obs,opts,index):
 try:
  o=opts[index];me=int(obs.current.yourIndex);c=_src(obs,opts,index)
  cid=int(getattr(c,'id',getattr(o,'cardId',0)) or 0)
  serial=int(getattr(c,'serial',getattr(o,'serial',-1)) or -1)
  owner_raw=getattr(o,'playerIndex',None)
  if owner_raw is None and c is not None:owner_raw=getattr(c,'playerIndex',None)
  owner=me if owner_raw is None else int(owner_raw)
  target=_own_target(obs,o);target_serial=int(getattr(target,'serial',-1) or -1)
  kind=_vg.ActionKind.OTHER
  role=_vg.BenchRole.OTHER
  hp=None;prizes=0;slot=False
  if _opt_type(o,_api.OptionType.PLAY) and cid in _H_RESET_IDS:
   kind=_vg.ActionKind.HAND_RESET
  elif _opt_type(o,_api.OptionType.PLAY) and cid==_V22_POFFIN_ID:
   kind=_vg.ActionKind.BASIC_SEARCH_ITEM
  elif _opt_type(o,_api.OptionType.PLAY):
   data=_base.card_table.get(cid)
   if data is not None and bool(getattr(data,'basic',False)):
    kind=_vg.ActionKind.BENCH_BASIC;slot=True;hp=int(getattr(data,'hp',0) or 0)
    prizes=3 if bool(getattr(data,'megaEx',False)) else 2 if bool(getattr(data,'ex',False)) else 1
    if cid==119:role=_vg.BenchRole.CORE_ATTACKER
    elif cid in (140,1071):role=_vg.BenchRole.HIGH_VALUE_SUPPORT
    elif cid==112:role=_vg.BenchRole.CONDITIONAL_DUPLICATE
  consumes=frozenset((serial,)) if kind is _vg.ActionKind.HAND_RESET and serial>=0 else frozenset()
  return _vg.ActionRef(kind,serial=serial,owner=owner,card_id=cid,card_name=str(getattr(_base.card_table.get(cid),'name','')),target_serial=target_serial,hp=hp,prize_value=prizes,consumes_bench_slot=slot,bench_role=role,consumes_commitment_serials=consumes)
 except Exception:
  return _vg.ActionRef(_vg.ActionKind.OTHER)

def _v22_terminal_threat(obs,opts,index,action):
 """Build P0 only from the exact public Grimmsnarl/Munkidori prize map."""
 try:
  if action.kind is not _vg.ActionKind.BENCH_BASIC or action.card_id!=235:return None
  st=obs.current;me=int(st.yourIndex);mine=st.players[me];op=st.players[1-me]
  active=mine.active[0] if mine.active and mine.active[0] is not None else None
  op_active=op.active[0] if op.active and op.active[0] is not None else None
  stadium=int(st.stadium[0].id) if st.stadium else 0
  if active is None or int(active.id)!=140 or int(active.hp)>30:return None
  if op_active is None or int(op_active.id)!=_V22_GRIM_ID or len(op.prize)!=3:return None
  if stadium!=1246:return None
  op_board=[p for p in list(op.active or [])+list(op.bench or []) if p is not None]
  dark_munk=any(int(p.id)==112 and 7 in _h_eids(p) for p in op_board)
  def damage_on(p):
   data=_base.card_table.get(int(p.id));base_hp=int(getattr(data,'hp',getattr(p,'hp',0)) or getattr(p,'hp',0))
   return max(0,int(getattr(p,'maxHp',base_hp) or base_hp)-int(p.hp))
  movable=max((damage_on(p) for p in op_board),default=0)
  attack=_base.attack_table.get(_V22_SHADOW_BULLET_ID)
  attached=[int(e.id) for e in list(op_active.energyCards or [])]
  required=list(getattr(attack,'energies',[]) or []) if attack is not None else []
  ready=bool(attack is not None and all(attached.count(e)>=required.count(e) for e in set(required) if int(e)!=0))
  revealed=bool(_history.opponent_attack_seen(1-me,_V22_SHADOW_BULLET_ID))
  text=str(getattr(attack,'text','')) if attack is not None else ''
  bench_text_proven=bool(revealed and '30 damage' in text and 'Benched' in text)
  pending=bool(dark_munk and movable>=30)
  # Conservatively call any legal attack a possible same-turn closeout; the
  # audited P0 state has none.  This prevents the guard from stealing a win.
  same_turn_win=any(_opt_type(o,_api.OptionType.ATTACK) for o in opts)
  safe_alt=any(i!=index and not (_opt_type(o,_api.OptionType.PLAY) and _cid(obs,opts,i)==235) for i,o in enumerate(opts))
  return _vg.TerminalPrizeThreat(
   opponent_prizes_remaining=len(op.prize),pending_non_attack_ko_prizes=2,
   pending_ko_proof=_vg.Proof.PROVEN if pending else _vg.Proof.UNKNOWN,
   attack_ready_now=ready,attack_available_after_pending_ko=ready,
   observed_bench_damage=30,bench_damage_proof=_vg.Proof.PROVEN if bench_text_proven else _vg.Proof.UNKNOWN,
   candidate_targetable_on_bench=True,candidate_stays_at_or_below_hp_until_opponent_attack=(action.hp==30),
   safe_alternative_exists=safe_alt,same_turn_win_available=same_turn_win,
  )
 except Exception:return None

def _v22_search_facts(obs,action):
 try:
  if action.kind is not _vg.ActionKind.BASIC_SEARCH_ITEM:return None
  mine=obs.current.players[obs.current.yourIndex]
  dreepy_out=len(_v22_visible_serials(mine,119))
  dreepy_upper=max(0,4-dreepy_out)
  repeated=bool(_history.poffin_resolutions_this_turn>0)
  return _vg.SearchFacts(
   # A first empty search can still have an intentional shuffle purpose that
   # this prefix adapter cannot disprove.  Disable runtime P3_NO_TARGET by
   # leaving the literal eligible bound unknown; deploy only the proven
   # same-turn redundant-empty predicate.
   eligible_target_upper_bound=None,
   productive_target_upper_bound=dreepy_upper,
   target_bound_proof=_vg.Proof.PROVEN if repeated and dreepy_upper==0 else _vg.Proof.UNKNOWN,
   same_search_resolved_this_turn=repeated,
   deck_order_information_since_last_shuffle=bool(_history.deck_order_information_since_last_poffin),
   explicit_shuffle_benefit=False,item_has_other_effect=False,
  )
 except Exception:return None

def _v22_pre_reset(obs,opts,action):
 commitments=[];shadow=set()
 try:
  if action.kind is not _vg.ActionKind.HAND_RESET:return (),()
  hand_serials={int(c.serial) for c in list(obs.current.players[obs.current.yourIndex].hand or [])}
  legal_by_serial={}
  for i in range(len(opts)):
   c=_src(obs,opts,i)
   if c is not None:legal_by_serial.setdefault(int(getattr(c,'serial',-1)),[]).append(i)
  for serial,meta in _history.pending_recon().items():
   # A selected Pad is productive only if a previously exposed, non-Rule-Box
   # Dreepy/Drakloak serial is still known to be in the deck.  Dragapult ex is
   # not a legal Pad target.  In 93140673 the prefix proves no such target, so
   # that former "verified" case is correctly retained as shadow-only.
   is_pad_before_lillie=bool(int(meta.get('card_id',0))==_V22_POKE_PAD_ID and action.card_id==1227 and serial in hand_serials and serial in legal_by_serial)
   # Shadow-only in v22: even when a legal target is known, playing Pad and
   # then Lillie simply shuffles the fetched Pokemon again unless a complete
   # consume-before-reset chain is also proven.  That chain is not tracked.
   proven=False
   if is_pad_before_lillie:shadow.add('v22_P1_selected_pad_shadow')
   commitments.append(_vg.PreResetCommitment(
    serial=serial,kind=_vg.CommitmentKind.SELECTED_SERIAL,selected_this_turn=True,
    unconsumed=True,in_hand=serial in hand_serials,will_be_lost_by_reset=True,
    legal_use_now=serial in legal_by_serial,reset_remains_legal_after_use=False,
    productive_use_proof=_vg.Proof.UNKNOWN,
    description='consume Recon-selected Poke Pad before Lillie' if proven else '',
   ))
  # The audited Psychic -> backup Dreepy attachment is shadow evidence only.
  # The prefix proves Petty Grudge becomes legal, but not that the target
  # survives every opponent response, so continuity_gain_proof stays ESTIMATED.
  pending=_history.pending_recon()
  selected_reset=action.serial in pending and int(pending[action.serial].get('card_id',0))==1213
  if action.card_id==1213 and selected_reset and int(getattr(obs.current,'turn',-1))==4:
   active=obs.current.players[obs.current.yourIndex].active[0] if obs.current.players[obs.current.yourIndex].active else None
   for i,o in enumerate(opts):
    c=_src(obs,opts,i);target=_own_target(obs,o)
    if (_opt_type(o,_api.OptionType.ATTACH) and c is not None and target is not None and int(getattr(c,'id',0))==5 and int(target.id)==119 and not list(target.energyCards or []) and active is not None and int(active.id)==120 and {2,5}.issubset(_h_eids(active))):
     shadow.add('v22_P1_attach_shadow')
     commitments.append(_vg.PreResetCommitment(serial=int(c.serial),kind=_vg.CommitmentKind.ATTACK_CONTINUITY_ATTACH,legal_use_now=True,reset_remains_legal_after_use=True,continuity_gain_proof=_vg.Proof.ESTIMATED,enables_next_own_turn_attack=True,description='shadow: Psychic to backup Dreepy'))
     break
 except Exception:pass
 return tuple(commitments),tuple(sorted(shadow))

def _v22_bench_plan(obs,opts,action):
 try:
  sel=obs.select;effect=int(getattr(getattr(sel,'effect',None),'id',0) or 0)
  if effect!=_V22_STRETCHER_ID or sel.context!=_api.SelectContext.TO_HAND:return None
  if action.card_id is None:
   return None
  data=_base.card_table.get(int(action.card_id));mine=obs.current.players[obs.current.yourIndex]
  alternatives=[]
  for i in range(len(opts)):
   c=_src(obs,opts,i)
   if c is not None and int(getattr(c,'id',0) or 0) in (2,5,7):alternatives.append(int(getattr(c,'serial',-1)))
  return _vg.BenchPlan(capacity=int(mine.benchMax),occupied=len(mine.bench),recovery_target_requires_bench_slot=bool(data is not None and getattr(data,'basic',False)),can_free_bench_slot_before_target_use=False,target_has_nonbench_productive_use=False,must_consume_target_this_turn=False,productive_alternative_serials=tuple(x for x in alternatives if x>=0))
 except Exception:return None

def _v22_guard_input(obs,opts,index):
 action=_v22_action_ref(obs,opts,index)
 # Stretcher targets are CARD options rather than PLAY options.
 try:
  if int(getattr(getattr(obs.select,'effect',None),'id',0) or 0)==_V22_STRETCHER_ID and obs.select.context==_api.SelectContext.TO_HAND:
   action=_vg.ActionRef(_vg.ActionKind.RECOVERY_TARGET,serial=action.serial,owner=action.owner,card_id=action.card_id,card_name=action.card_name,target_serial=action.serial)
 except Exception:pass
 commitments,shadow=_v22_pre_reset(obs,opts,action)
 return _vg.GuardInput(action,terminal_threat=_v22_terminal_threat(obs,opts,index,action),pre_reset_commitments=commitments,bench_plan=_v22_bench_plan(obs,opts,action),search_facts=_v22_search_facts(obs,action)),shadow

def _v22_unblocked_fallback(obs,opts,blocked,terminal):
 """Choose a deterministic semantic fallback without a second policy call.

 Re-entering either UnifiedController or foundation_agent mutates their turn
 state.  A fixed progress ordering is slower-changing and keeps P0/P3 hard
 blocks fail-closed under every load order.
 """
 if terminal:
  # Fill the final slot with a body that survives the proven 30 Bench damage;
  # Dreepy is the core line and also prevents a later Budew re-proposal.
  safe=[]
  for i,o in enumerate(opts):
   if i in blocked or not _opt_type(o,_api.OptionType.PLAY):continue
   cid=_cid(obs,opts,i);data=_base.card_table.get(cid)
   if data is not None and bool(getattr(data,'basic',False)) and int(getattr(data,'hp',0) or 0)>30:
    safe.append((0 if cid==119 else 1,0 if not getattr(data,'ex',False) else 1,i))
  if safe:return [min(safe)[2]]
  # Consume a Drakloak draw only if no safe body can close the liability slot.
  q=next((i for i,o in enumerate(opts) if i not in blocked and _opt_type(o,_api.OptionType.ABILITY) and _cid(obs,opts,i)==120),None)
  if q is not None:return [q]
  q=next((i for i,o in enumerate(opts) if i not in blocked and _opt_type(o,_api.OptionType.EVOLVE)),None)
  if q is not None:return [q]
  # Then prefer a nonduplicate F/P attachment to a Dragapult line.
  for i,o in enumerate(opts):
   if i in blocked or not _opt_type(o,_api.OptionType.ATTACH):continue
   eid=_cid(obs,opts,i);target=_own_target(obs,o)
   if target is not None and int(target.id) in _H_DRAG_IDS and eid in (2,5) and eid not in _h_eids(target):return [i]
  order=(_api.OptionType.ABILITY,_api.OptionType.PLAY,_api.OptionType.ATTACH,_api.OptionType.RETREAT,_api.OptionType.ATTACK,_api.OptionType.END)
 else:
  # A redundant search is not permission to end the branch early.  Prefer
  # only audited, productive setup actions; broad ATTACH/ABILITY ordering
  # previously spent Fire/Psychic on Fezandipiti or attacked before drawing.
  for want in (120,140,112):
   q=next((i for i,o in enumerate(opts) if i not in blocked and _opt_type(o,_api.OptionType.ABILITY) and _cid(obs,opts,i)==want and _cid(obs,opts,i)!=1259),None)
   if q is not None:return [q]
  q=next((i for i,o in enumerate(opts) if i not in blocked and _opt_type(o,_api.OptionType.EVOLVE)),None)
  if q is not None:return [q]
  productive_attach=[]
  for i,o in enumerate(opts):
   ...
   energies=_h_eids(target)
   if eid in (2,5) and int(target.id) in _H_DRAG_IDS and eid not in energies:
    other=5 if eid==2 else 2
    productive_attach.append((0 if other in energies else 1,i))
   elif eid==7 and int(target.id)==112 and eid not in energies:
    productive_attach.append((2,i))
  if productive_attach:return [min(productive_attach)[1]]
  play_priority=(1152,1097,1121,1198,1246,1120)
  for want in play_priority:
   q=next((i for i,o in enumerate(opts) if i not in blocked and _opt_type(o,_api.OptionType.PLAY) and _cid(obs,opts,i)==want),None)
   if q is not None:return [q]
  q=next((i for i,o in enumerate(opts) if i not in blocked and _opt_type(o,_api.OptionType.PLAY) and _cid(obs,opts,i) not in _H_RESET_IDS),None)
  if q is not None:return [q]
  attacks=[]
  for i,o in enumerate(opts):
   if i in blocked or not _opt_type(o,_api.OptionType.ATTACK):continue
   aid=int(getattr(o,'attackId',0) or 0);data=_base.attack_table.get(aid)
   damage=int(getattr(data,'damage',0) or 0) if data is not None else 0
   rank=0 if aid==154 else 1 if aid==152 else 2
   attacks.append((rank,-damage,aid,i))
  if attacks:return [min(attacks)[3]]
  order=(_api.OptionType.RETREAT,_api.OptionType.END)
 for kind in order:
  q=next((i for i,o in enumerate(opts) if i not in blocked and _opt_type(o,kind)),None)
  if q is not None:return [q]
 q=next((i for i in range(len(opts)) if i not in blocked),None)
 return [q] if q is not None else None

class _V22PatchedAction(list):
 """List-compatible result carrying semantic options that must stay blocked."""
 def __init__(self,indices=(),blocked=()):
  super().__init__(indices)
  self.blocked_indices=frozenset(blocked)

def _v22_guard_patch(observation,chosen):
 """Apply hard/defer guards after v21, with terminal fail-safe behavior."""
 try:
  if not isinstance(chosen,list) or len(chosen)!=1 or not observation.get('select'):return chosen
  obs=_api.to_observation_class(observation);sel=obs.select
  if obs.current is None or sel is None:return chosen
  ...
  verdicts={};shadows={}
  for i in range(len(opts)):
   inp,shadow=_v22_guard_input(obs,opts,i);verdicts[i]=_vg.evaluate(inp);shadows[i]=shadow
  key=(int(obs.current.turn),int(obs.current.turnActionCount),int(sel.context))
  first=key not in _history.v22_guard_eval_fingerprints
  if first:_history.v22_guard_eval_fingerprints.add(key)
  if first:
   for name in sorted({name for names in shadows.values() for name in names}):_h_bump(name)
  if first:
   for verdict in verdicts.values():
    for hit in verdict.hits:_h_bump('v22_'+hit.guard_id.value)

  chosen_verdict=verdicts[ci]
  if chosen_verdict.disposition is _vg.Disposition.DEFER:
   required=[]
   for hit in chosen_verdict.hits:required.extend(hit.required_serials)
   me=int(obs.current.yourIndex)
   for serial in required:
    for i in range(len(opts)):
     ref=_v22_action_ref(obs,opts,i)
     if ref.serial==serial and ref.owner==me and ref.kind is not _vg.ActionKind.HAND_RESET:
      return [i]
   # Nonterminal sequencing uncertainty fails open to v21.
   return chosen

  blocked={i for i,v in verdicts.items() if v.disposition is _vg.Disposition.BLOCK}
  if ci in blocked:
   terminal=any(any(h.guard_id is _vg.GuardId.P0_TERMINAL_LOW_HP_BENCH for h in verdicts[i].hits) for i in blocked)
   alternative=_v22_unblocked_fallback(obs,opts,blocked,terminal)
   if alternative:return _V22PatchedAction(alternative,blocked)
   # This is unreachable for a proven P0/P3 hit because both predicates
   # require a safe/non-search alternative.  Preserve the prohibited set so
   # validation cannot resurrect the blocked original if malformed input ever
   # violates that contract.
   return _V22PatchedAction((),blocked)
  # P2 DEMOTE is shadow-only in this release; deployment keeps the v21 choice.
  return chosen
 except Exception:return chosen
...
def _validated_action(observation,candidate,fallback):
 try:
  ...
  prohibited=frozenset(getattr(candidate,'blocked_indices',()))
  def valid(x):return isinstance(x,list) and lo<=len(x)<=hi and len(set(x))==len(x) and all(isinstance(i,int) and 0<=i<n and i not in prohibited for i in x)
  if valid(candidate):return candidate
  if valid(fallback):
   _h_bump('invalid_guard_fallback');return fallback
  out=[i for i in range(n) if i not in prohibited][:lo]
...
def get_plan_stats():
 ...
 out.update(_temporal.get_stats())
 out.update(_semantic_llm.get_stats())
 out.update(_league.get_stats())
...
def agent(observation:dict)->list[int]:
 ...
 if not observation.get('select') and observation.get('current') is None:
  ...
  try:_temporal.stats.clear()
  except Exception:pass
  try:_semantic_llm.reset()
  except Exception:pass
  try:_league.stats.clear();_league.reset()
  except Exception:pass
 ...
 try:
  ...
  chosen=_temporal.rerank(observation,chosen,_history,_tname,_tconf)
  chosen=_semantic_llm.rerank(observation,chosen,_history,_tname,_tconf)
 except Exception:pass
 original_chosen=list(chosen) if isinstance(chosen,list) else chosen
 chosen=_causal_history_guards(observation,chosen)
 try:chosen=_league.rerank(observation,chosen,_history)
 except Exception:pass
 chosen=_v22_guard_patch(observation,chosen)
