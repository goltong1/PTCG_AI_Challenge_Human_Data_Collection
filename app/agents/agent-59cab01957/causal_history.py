...
class CausalHistory:
    ...
    def reset(self):
        ...
        self.observed_attack_ids_by_player = {}
        # Exact own-deck serials revealed by a search callback.  These facts
        # survive shuffles, but are removed as soon as a public movement shows
        # the serial leaving the deck.
        self.known_own_deck = {}
    ...
    def _reset_turn_facts(self):
        ...
        # v22 causal ledger.  Every set/dict is keyed by semantic serial, never
        # by the current legal-option array index.
        self.recon_selected_serials = {}
        self.consumed_serials_this_turn = set()
        self.poffin_play_serials_this_turn = set()
        self.poffin_resolutions_this_turn = 0
        self.poffin_fetched_serials_this_turn = set()
        self.deck_order_information_since_last_poffin = False
        self.recorded_action_fingerprints = set()
        self.v22_guard_eval_fingerprints = set()
    ...
    def _ingest_log(self, log):
        ...
        if kind == _ATTACK and player >= 0:
            self.observed_attack_ids_by_player.setdefault(player, set()).add(
        ...
        if mine:
            from_area = _enum_int(getattr(log, "fromArea", None))
            to_area = _enum_int(getattr(log, "toArea", None))
            if kind == _DRAW or (kind == _MOVE_CARD and from_area == 1):
                self.known_own_deck.pop(serial, None)
            if kind == _MOVE_CARD and to_area == 1 and serial >= 0 and card_id > 0:
                self.known_own_deck[serial] = card_id
            if kind in (_PLAY, _ATTACH, _EVOLVE):
                self.known_own_deck.pop(serial, None)
            event_key = (
                kind, card_id, serial,
                _int(getattr(log, "attackId", None), 0),
    ...
    def opponent_attack_seen(self, opponent_index, attack_id):
        return _int(attack_id, 0) in self.observed_attack_ids_by_player.get(
            _int(opponent_index, -1), set()
        )

    def known_own_deck_has(self, card_ids):
        wanted = {_int(card_id, 0) for card_id in card_ids}
        return any(card_id in wanted for card_id in self.known_own_deck.values())
    ...
    def record_action(self, obs, indices, source_resolver=None):
        ...
        # A deck-search callback publicly proves that every enumerated serial
        # is in our deck at this prefix.  Record the whole semantic option set,
        # not merely the selected index, so shadow audits retain exact causal
        # deck knowledge without reading hidden replay state.
        for option_index, option in enumerate(options):
            if _enum_int(getattr(option, "area", None)) != 1:
                continue
            source = None
            if source_resolver is not None:
                try:
                    source = source_resolver(obs, options, option_index)
                except Exception:
                    source = None
            serial = _int(getattr(source, "serial", None), -1)
            card_id = _int(getattr(source, "id", None), 0)
            if serial >= 0 and card_id > 0:
                self.known_own_deck[serial] = card_id
        ...
        for index in list(indices or []):
            ...
            selected.append((index, option, kind, source, card_id, serial, target))

        # CABT can repeat a callback.  Make every ledger mutation idempotent
        # with a causal event fingerprint containing semantic serials/owner.
        fingerprint = (
            turn,
            action_count,
            context,
            _int(getattr(effect, "id", None), 0),
            tuple(
                (
                    kind,
                    serial,
                    _int(getattr(source, "playerIndex", None), self.my_index),
                    _int(getattr(target, "serial", None), -1),
                    _int(getattr(option, "attackId", None), 0),
                )
                for _, option, kind, source, _, serial, target in selected
            ),
        )
        if fingerprint in self.recorded_action_fingerprints:
            return
        self.recorded_action_fingerprints.add(fingerprint)

        for index, option, kind, source, card_id, serial, target in selected:
            semantic = {
            ...
            if kind == _OPT_PLAY:
                if serial >= 0:
                    self.consumed_serials_this_turn.add(serial)
                ...
                if card_id == _POFFIN_ID and serial not in self.poffin_play_serials_this_turn:
                    self.poffin_play_serials_this_turn.add(serial)
                    self.poffin_resolutions_this_turn += 1
                    self.deck_order_information_since_last_poffin = False
                if self.reset_sequence_phase == 1 and card_id == self.reset_sequence_reset_id:
                    self.reset_sequence_phase = 2
            elif kind in (_OPT_ATTACH, _OPT_EVOLVE):
                if serial >= 0:
                    self.consumed_serials_this_turn.add(serial)
            ...
            if kind == _OPT_CARD and context == _CTX_TO_HAND and _int(getattr(effect, "id", None), 0) == _DRAKLOAK_ID:
                if serial >= 0:
                    self.recon_selected_serials[serial] = {
                        "card_id": card_id,
                        "turn": turn,
                        "turn_action_count": action_count,
                    }
                    if self.poffin_resolutions_this_turn > 0:
                        self.deck_order_information_since_last_poffin = True

            if kind == _OPT_CARD and context == _CTX_TO_BENCH and _int(getattr(effect, "id", None), 0) == _POFFIN_ID:
                if serial >= 0:
                    self.poffin_fetched_serials_this_turn.add(serial)

            # Any exact card selected from the deck has left it by the time
            # the next MAIN callback is generated.
            if kind == _OPT_CARD and _enum_int(getattr(option, "area", None)) == 1:
                self.known_own_deck.pop(serial, None)
    ...
    def pending_recon(self):
        return {
            serial: dict(meta)
            for serial, meta in self.recon_selected_serials.items()
            if serial not in self.consumed_serials_this_turn
        }
    ...
    def stats(self):
        return {
            ...
            "recon_pending_this_turn": len(self.pending_recon()),
            "consumed_serials_this_turn": len(self.consumed_serials_this_turn),
            "poffin_resolutions_this_turn": self.poffin_resolutions_this_turn,
            "poffin_fetched_this_turn": len(self.poffin_fetched_serials_this_turn),
            "known_own_deck_serials": len(self.known_own_deck),
            "deck_info_since_poffin": int(self.deck_order_information_since_last_poffin),
        }
