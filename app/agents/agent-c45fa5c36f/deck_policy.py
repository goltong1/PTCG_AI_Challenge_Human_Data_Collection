...
def foundation_agent(obs_dict: dict) -> list[int]:
    ...
    def _primary_key(p):
        ...
        active_bonus = 0 if p in my_state.active else 2
        bench_counter_safe = 1
        active_attack_safe = 1
        return (bench_counter_safe, active_attack_safe, required, active_bonus, stage, len(p.energies), p.hp)
    ...
    def attach_score(attach_id: int, pokemon: Pokemon, active: bool) -> int:
        ...
        if active_support_stranded and active and (active_pokemon is not None) and (pokemon.serial == active_pokemon.serial) and (energy_count < card_table[pokemon.id].retreatCost):
            ...
            if attach_id == Basic_Darkness_Energy and pokemon.id in (Munkidori, Yveltal):
                return 36200
        # Duskull/Dusclops/Dusknoir are counter-placement pieces in this list;
        # their job does not require manual Energy.  Preserve F/P for the next
        # Dragapult and Darkness for Munkidori instead of charging a front Dusk.
        if pokemon.id in (131, 132, 133):
            return -1
            ...
            if card != None:
                ...
                if context == SelectContext.SWITCH or context == SelectContext.TO_ACTIVE or context == SelectContext.SETUP_ACTIVE_POKEMON:
                    if o.playerIndex == my_index:
                        if card.id == Dreepy:
                            # Forced promotion: a Dreepy is the charger, not the shield.
                            # Keep the old setup/switch behavior, but protect it after a KO.
                            score += (1000 - 5000*energy_count) if context == SelectContext.TO_ACTIVE else 10000
                        elif card.id == Drakloak:
                            if context == SelectContext.TO_ACTIVE:
                                ids={e.id for e in card.energyCards}
                                drag_in_hand=hand_ids_now[Dragapult_ex] > 0
                                score += 65000 if ({Basic_Fire_Energy,Basic_Psychic_Energy}.issubset(ids) and drag_in_hand) else (2000 - 4000*energy_count)
                            elif energy_count >= 1:
                                score += 20000
                            else:
                                score -= 10000
                            ...
                            score += 50000
                        elif card.id == 131 and context == SelectContext.TO_ACTIVE:
                            # Duskull's counter-placement line needs no manual Energy and
                            # is the preferred one-Prize front shield while Bench charges.
                            score += 38000
                        elif card.id == Chi_Yu:
                            score += 1000
                            ...
                            if context == SelectContext.TO_ACTIVE and not any(e.id == Basic_Darkness_Energy for e in card.energyCards):
                                score += 24000
                            else:
                                score += -1500
                        elif card.id == Budew:
