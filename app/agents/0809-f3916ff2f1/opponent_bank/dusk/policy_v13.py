...
def no_ability_counter(pokemon: Pokemon) -> bool:
    """Ability-placed damage counters are not blocked by attack-only protection or Mist Energy."""
    return False
...
def agent(obs_dict: dict) -> list[int]:
    ...
    is_zoroark_match = bool(seen_opponent_ids & {292,293,303,906})
    is_crustle_match = bool((seen_opponent_ids & {756}) or ((seen_opponent_ids & {344,345}) and not is_zoroark_match))
    ...
    is_archaludon_match = bool(seen_opponent_ids & {169,190,666})
    is_starmie_match = bool(seen_opponent_ids & {860,861,1030,1031})
    opponent_profile = ('zoroark' if is_zoroark_match else 'crustle' if is_crustle_match else 'lucario' if is_lucario_match else
    ...
    efficiency_profile = is_alakazam_match or is_marnie_match or is_lucario_match or is_rocket_match or is_zoroark_match
    ...
    urgent_attack = (is_lucario_match or is_rocket_match or is_zoroark_match) and (len(op_state.prize) <= 2 or (opponent_ready_attacker and likely_boss))
    target_priority_ids = {'dragapult': {Dreepy: 7000, Drakloak: 10000, Munkidori: 8000}, 'lucario': {677:8500,673:6500,675:4500,676:4500}, 'alakazam': {741:7500,742:10500}, 'marnie': {646:7600,647:10500,648:12000,104:11800,860:7000,Munkidori:11200}, 'zoroark': {292:9800,293:10500,344:9000,862:7600,906:8200,303:7600}, 'starmie': {1030:8500,860:7500,Munkidori:7000}, 'crustle': {344:12000}}.get(opponent_profile,{})
    ...
    stall_caps = {'dragapult': 4 if mirror_profile == 1 else 3, 'lucario': 3,
                  'alakazam': 2, 'marnie': 4, 'zoroark': 3, 'starmie': 2, 'crustle': 1,
                  'rocket': 1, 'unknown': 1}
    stall_cap = stall_caps.get(opponent_profile, 1)
    if is_archaludon_match:
        imminent_phantom_line = any(
            p.id == Drakloak and not p.appearThisTurn
            and Basic_Fire_Energy in {e.id for e in p.energyCards}
            and Basic_Psychic_Energy in {e.id for e in p.energyCards}
            for p in list(my_state.active) + list(my_state.bench) if p is not None
        )
        archaludon_engine_online = any(
            p.id in (190, 666) and len(p.energyCards) >= 1 for p in visible_opponent
        )
        stall_setup = bool(active_id == Budew and not ready_dragapult and state.turn <= 7
                           ...
                           and not imminent_phantom_line
                           and not (archaludon_engine_online and state.turn >= 5))
    else:
        stall_setup = bool(active_id == Budew and not ready_dragapult and state.turn <= 7
                           and dreepy_line_count >= 1 and budew_stall_uses < stall_cap)
    ...
    charged_drakloak_can_evolve = any(
        p.id == Drakloak and not p.appearThisTurn
        and Basic_Fire_Energy in {e.id for e in p.energyCards}
        and Basic_Psychic_Energy in {e.id for e in p.energyCards}
        for p in list(my_state.active) + list(my_state.bench) if p is not None
    )
    immediate_phantom_search = bool(charged_drakloak_can_evolve and deck_counts[Dragapult_ex] > 0)

    # Low-cost active Dusk engine for the Marnie matchup. Dusclops is the
    # routine combo piece; Dusknoir is used when already available (or fetched
    # by a Poké Pad already in hand) to break Grimmsnarl ex with 130+200.
    own_field = [p for p in list(my_state.active) + list(my_state.bench) if p is not None]
    can_evolve_dusclops = any(p.id == Dusclops and not p.appearThisTurn for p in own_field)
    can_evolve_duskull = any(p.id == Duskull and not p.appearThisTurn for p in own_field)
    dusk_line_count = sum(1 for p in own_field if p.id in (Duskull, Dusclops, Dusknoir))
    dusknoir_in_hand = any(c.id == Dusknoir for c in (my_state.hand or []))
    poke_pad_in_hand = any(c.id == Poke_Pad for c in (my_state.hand or []))
    dawn_in_hand = any(c.id == Dawn for c in (my_state.hand or []))
    active_phantom_ready = bool(active_pokemon is not None and _phantom_ready(active_pokemon)
                                and not my_state.asleep and not my_state.paralyzed)
    active_target = op_state.active[0] if op_state.active and op_state.active[0] is not None else None
    marnie_support_ids = {646, 647, 104, 860, Munkidori}

    def _dusk_closeout_plan(blast: int):
        best = {'score': -1, 'target': None, 'wins': False, 'go': False}
        active_prizes = 0
        if active_phantom_ready and active_target is not None and not no_damage_dex(active_target.id) and active_target.hp <= 200:
            active_prizes = prize_count(active_target, True)
        for area, group in ((AreaType.ACTIVE, op_state.active), (AreaType.BENCH, op_state.bench)):
            for target in group:
                if target is None or no_ability_counter(target):
                    continue
                direct = target.hp <= blast
                active_combo = bool(active_phantom_ready and area == AreaType.ACTIVE and target.hp <= blast + 200
                                    and not no_damage_dex(target.id))
                bench_combo = bool(active_phantom_ready and area == AreaType.BENCH and target.hp <= blast + 60)
                if not (direct or active_combo or bench_combo):
                    continue
                prizes = prize_count(target, False) + (active_prizes if area == AreaType.BENCH else 0)
                wins = prizes >= len(my_state.prize)
                if blast == 50:
                    support_combo = is_marnie_match and target.id in marnie_support_ids and (direct or bench_combo)
                    go = wins or support_combo or prizes >= 2
                    bonus = 150000 if support_combo else 0
                else:
                    grim_break = is_marnie_match and target.id == 648 and area == AreaType.ACTIVE and active_combo
                    direct_finish = is_marnie_match and target.id == 648 and direct
                    high_value_combo = bool(area == AreaType.ACTIVE and active_combo and prize_count(target, False) >= 2 and len(op_state.prize) >= 2)
                    high_value_direct = bool(direct and prize_count(target, False) >= 2 and len(op_state.prize) >= 2)
                    go = wins or grim_break or direct_finish or high_value_combo or high_value_direct
                    bonus = 260000 if grim_break else (210000 if high_value_combo else (160000 if direct_finish or high_value_direct else 0))
                score = -1 if not go else 170000 + prizes * 80000 + pokemon_score(target, False) + bonus
                if score > best['score']:
                    best = {'score': score, 'target': target, 'wins': wins, 'go': go}
        return best

    dusclops_closeout = _dusk_closeout_plan(50)
    dusknoir_closeout = _dusk_closeout_plan(130)
    high_value_noir_route = bool(active_target is not None and prize_count(active_target, False) >= 2
        and ((active_phantom_ready and 200 < active_target.hp <= 330 and not no_damage_dex(active_target.id)) or active_target.hp <= 130)
        and len(op_state.prize) >= 2)
    grim_noir_route = bool(is_marnie_match and active_target is not None and active_target.id == 648 and high_value_noir_route)
    dusclops_any = any(p.id == Dusclops for p in own_field)
    search_dusknoir_now = bool((high_value_noir_route if is_marnie_match else dusknoir_closeout['go']) and dusclops_any and deck_counts[Dusknoir] > 0 and (poke_pad_in_hand or dawn_in_hand))
    preserve_dusclops_for_noir = bool((high_value_noir_route if is_marnie_match else dusknoir_closeout['go']) and dusclops_any and (dusknoir_in_hand or search_dusknoir_now))
    immediate_dusknoir_closeout = bool(dusknoir_in_hand and can_evolve_dusclops and dusknoir_closeout['go'])
    crustle_future_dusclops = bool(field_counts[Dusclops] > 0 or hand_counts[Dusclops] > 0 or (field_counts[Duskull] > 0 and deck_counts[Dusclops] > 0))
    crustle_future_dusknoir = bool(field_counts[Dusclops] > 0 and (hand_counts[Dusknoir] > 0 or deck_counts[Dusknoir] > 0))
    ...
    def hand_score(id: int, ignore_count: bool):
        ...
        if id == Dreepy:
            ...
            score = 18000 if (is_crustle_match or ready_dragapult) and dusk_line_count == 0 and len(my_state.bench) < 5 else UNNECESSARY
        elif id == Dusclops:
            score = 32000 if can_evolve_duskull and (is_crustle_match or ready_dragapult) else UNNECESSARY
        elif id == Dusknoir:
            score = 67000 if immediate_dusknoir_closeout else (35000 if preserve_dusclops_for_noir else 1200)
        elif id == Munkidori:
            ...
            if is_crustle_match and opponent_crustle_active and (can_main_attack or bench_attacker) and any(p.id != 345 for p in op_state.bench if p is not None):
                score = 115000
            elif is_crustle_match and opponent_dwebble_bench > 0 and (can_attack or bench_attacker):
                score = 90000
            ...
            if search_dusknoir_now:
                score = 69000
            else:
                core_value = 26000 if (can_evolve_dreepy or can_evolve_drakloak) else 0
                setup_value = 9000 if (dreepy_line_count < 2 and len(my_state.bench) < 5) else 0
                score = core_value + setup_value + (7000 if my_state.handCount <= 4 else 0)
        elif id == Crispin:
    ...
    budew_reentry = (active_id != Budew and field_counts[Budew] >= 1 and state.turn >= 2
                     and (phase == MachineState.STALL_SETUP if is_alakazam_match else True))
    do_switch = (not can_main_attack and (bench_attacker or wall_attacker_ready or budew_reentry))
    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
            ...
            if card != None:
                ...
                if (context == SelectContext.SWITCH
                    ...
                    if effect_card_id in (Poke_Pad, Ultra_Ball):
                        ...
                        elif card.id == Dragapult_ex and can_evolve_drakloak: score += 160000 if immediate_phantom_search else 88000
                        ...
                        elif card.id == Dusknoir and search_dusknoir_now: score += 205000
                    if effect_card_id == Dawn:
                        if card.id == Dreepy and dreepy_line_count < 2:
                            score += 100000
                        elif card.id == Drakloak and can_evolve_dreepy:
                            score += 115000
                        elif card.id == Dragapult_ex and can_evolve_drakloak:
                            score += 125000
                        elif card.id == Dusknoir and search_dusknoir_now:
                            score += 205000
                        else:
                            score -= 40000
                    ...
                    if hp > 0:
                        ...
                        if context == SelectContext.DAMAGE_COUNTER and effect_card_id in (Dusclops, Dusknoir):
                            closeout = dusclops_closeout if effect_card_id == Dusclops else dusknoir_closeout
                            blast = 50 if effect_card_id == Dusclops else 130
                            if no_ability_counter(card):
                                score = -1
                            elif closeout['target'] is not None and card.serial == closeout['target'].serial:
                                score += 600000
                            elif closeout['target'] is not None:
                                score -= 300000
                            if score >= 0 and hp <= blast:
                                score += 300000 + prize_count(card, False) * 160000
                            elif score >= 0 and o.area == AreaType.ACTIVE and active_id == Dragapult_ex and any(p.id == Dragapult_ex and Basic_Fire_Energy in {e.id for e in p.energyCards} and Basic_Psychic_Energy in {e.id for e in p.energyCards} for p in my_state.active if p is not None) and 200 < hp <= blast + 200:
                                score += 180000 + prize_count(card, False) * 90000
                            elif score >= 0 and o.area == AreaType.BENCH and active_id == Dragapult_ex and any(p.id == Dragapult_ex and Basic_Fire_Energy in {e.id for e in p.energyCards} and Basic_Psychic_Energy in {e.id for e in p.energyCards} for p in my_state.active if p is not None) and 60 < hp <= blast + 60:
                                score += 150000 + prize_count(card, False) * 80000
                        if context == SelectContext.DAMAGE_COUNTER:
                            ...
                            if is_dragapult_match:
                                if card.id in (Dreepy, Drakloak) and hp <= remain_damage:
                                    ...
                                    score += 130000
                            if is_crustle_match:
                                projected_hp = hp - remain_damage
                                target_prizes = prize_count(card, False)
                                if projected_hp > 0:
                                    if crustle_future_dusclops and projected_hp <= 50:
                                        score += 125000 + target_prizes * 70000
                                    elif crustle_future_dusknoir and projected_hp <= 130:
                                        score += 90000 + target_prizes * 65000
                                    if card.id in target_priority_ids and hp < card.maxHp:
                                        score += min(90000, target_priority_ids[card.id] * 5)
                            dusk_followup = field_counts[Dusclops] > 0 or hand_counts[Dusclops] > 0
                            if is_marnie_match and dusk_followup and card.id in marnie_support_ids:
                                prio = {104: 220000, Munkidori: 210000, 647: 195000, 646: 170000, 860: 165000}.get(card.id, 0)
                                if hp <= remain_damage:
                                    score += 260000 + prio
                                elif 50 < hp <= 50 + remain_damage:
                                    score += prio
            ...
            if card.id == Dreepy:
                ...
                active_energy = len(op_state.active[0].energyCards) if op_state.active and op_state.active[0] is not None else 0
                score = 40000 if op_energy > 0 and (ready_dragapult or active_energy >= 2 or len(op_state.prize) <= 3) else -1
            elif card.id == Dawn:
                score = 52500 if card.id == use_support and my_state.deckCount > 6 else -1
                ...
                if immediate_phantom_search and my_state.handCount >= 3:
                    score = 98000
                else:
                    score = None
                wall_target = is_crustle_match and field_counts[Chi_Yu] == 0 and deck_counts[Chi_Yu] > 0
                if score is None and missing_stage and (negative_hand_count >= 1 or disposable_hand_count >= 1) and my_state.handCount >= 3:
                    score = 53000 if phase == MachineState.MIRROR_CONTROL else 50500
                elif score is None and wall_target and (negative_hand_count >= 1 or disposable_hand_count >= 1) and my_state.handCount >= 3:
                    score = 53500
                elif score is None and main_pokemon_count < 2 and negative_hand_count >= 1:
                    score = 48000
                elif score is None and negative_hand_count >= 2:
                    score = 44000
                elif score is None:
                    score = -1
            elif card.id == Poke_Pad:
                if immediate_phantom_search:
                    score = 96000
                elif efficiency_profile:
            ...
            pokemon = get_card(obs, o.inPlayArea, o.inPlayIndex, my_index)
            evolution = get_card(obs, o.area, o.index, my_index)
            score += len(pokemon.energies)
            if evolution is not None and evolution.id == Dusknoir:
                score = 180000 if immediate_dusknoir_closeout else -1
            elif evolution is not None and evolution.id == Dusclops:
                if not (is_crustle_match or ready_dragapult or high_value_noir_route):
                    score = -1
                else:
                    score += 85000
            elif pokemon.id == Dreepy:
            ...
            card = get_card(obs, o.area, o.index, my_index)
            if card.id == Dusknoir:
                score = dusknoir_closeout['score'] if (immediate_dusknoir_closeout or dusknoir_closeout['wins']) else -1
            elif card.id == Dusclops:
                if preserve_dusclops_for_noir:
                    score = -1
                elif dusclops_closeout['go']:
                    score = dusclops_closeout['score']
                else:
                    score = 40000
            elif card.id == Munkidori:
            ...
            if is_crustle_match and opponent_crustle_active and active_id == Dragapult_ex:
                if o.attackId == 154 and any(p is not None and not no_damage_counter(p) for p in op_state.bench):
                    score = 72000
                else:
                    score = -1
