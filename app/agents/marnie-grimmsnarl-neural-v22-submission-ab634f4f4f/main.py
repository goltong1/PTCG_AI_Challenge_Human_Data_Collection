"""Neural mixture-of-experts v22 Marnie's Grimmsnarl ex agent for CABT.

The public entry point is agent(observation: dict) -> list[int].
The submitted policy runs five independently trained Marnie policies and uses a compact neural network to choose the final legal action. The network is trained from replay and cross-play data; deterministic experts remain as safety candidates.
"""
...
_CORRECTION_PATH = "replay_corrections_v21.json"
if not os.path.exists(_CORRECTION_PATH):
    _CORRECTION_PATH = "/kaggle_simulations/agent/replay_corrections_v21.json"
try:
    ...
    if source == POKE_PAD:
        if plan.archetype == "alakazam" and field[GRIMMSNARL_EX] < 2:
            ...
            if cid == IMPIDIMP and line_count < 2:
                return 30500
        if (plan.archetype == "archaludon" and line_count >= 1
...
def _rule_strategy(obs: Observation) -> list[int]:
    state = obs.current
    select = obs.select
    me = state.yourIndex
    mine = state.players[me]
...
# Neural policy v22
# ---------------------------------------------------------------------------
# The rule policy above is retained as a teacher prior and deterministic safety
# fallback.  The submitted action is produced by a compact MLP that scores every
# legal option and predicts the number of options to select.  Local training can
# replace nn_policy_v22.npz without editing this file.
try:
    from experts import v19_policy as _EXPERT_V19
    from experts import v20_policy as _EXPERT_V20
    from experts import supplied_policy as _EXPERT_SUPPLIED
    from experts.v16_agent import main as _EXPERT_V16
    _EXTERNAL_EXPERTS = (_EXPERT_V19, _EXPERT_V20, _EXPERT_SUPPLIED, _EXPERT_V16)
except Exception:
    _EXTERNAL_EXPERTS = ()

_NN_MODEL_PATH = "nn_policy_v22.npz"
if not os.path.exists(_NN_MODEL_PATH):
    _NN_MODEL_PATH = "/kaggle_simulations/agent/nn_policy_v22.npz"
try:
    import numpy as _np
except Exception:
    _np = None
...
_NN_STATE_DIM = 128
_NN_OPTION_DIM = 96
_NN_INPUT_DIM = _NN_STATE_DIM + _NN_OPTION_DIM
_NN_MODEL: dict[str, Any] = {}
_NN_META: dict[str, Any] = {}
_NN_LAST_EXPERT = 2
_NN_LAST_ACTIONS: list[list[int]] = []


def _load_nn_policy() -> None:
    global _NN_MODEL, _NN_META
    if _np is None or not os.path.exists(_NN_MODEL_PATH):
        _NN_MODEL = {}
        return
    try:
        data = _np.load(_NN_MODEL_PATH, allow_pickle=False)
        required = {"w1", "b1", "w2", "b2", "wo", "bo", "cw1", "cb1", "cwo", "cbo", "ew1", "eb1", "ewo", "ebo"}
        if not required.issubset(set(data.files)):
            _NN_MODEL = {}
            return
        _NN_MODEL = {k: data[k].astype(_np.float32, copy=False) for k in required}
        for key in ("confidence_margin", "expert_confidence", "temperature", "version"):
            if key in data.files:
                value = data[key]
                _NN_META[key] = float(value.reshape(-1)[0]) if key != "version" else str(value.reshape(-1)[0])
    except Exception:
        _NN_MODEL = {}
...
def _load_expert_correction(filename: str) -> dict[str, Any]:
    path = filename
    ...
    try:
        ...
        return _REPLAY_CORRECTION


_EXPERT_NAMES = ("v19", "v20", "v21", "supplied", "v16")
_EXPERT_CORRECTIONS = (
    _load_expert_correction("replay_corrections_v19.json"),
    _load_expert_correction("replay_corrections_v20.json"),
    _REPLAY_CORRECTION,
)


def _nn_hash_slot(value: int, bins: int) -> tuple[int, float]:
    x = int(value or 0) & 0xFFFFFFFF
    x ^= x >> 16
    x = (x * 0x7FEB352D) & 0xFFFFFFFF
    x ^= x >> 15
    x = (x * 0x846CA68B) & 0xFFFFFFFF
    x ^= x >> 16
    return x % bins, (1.0 if (x & 1) else -1.0)


def _nn_add_hash(vec, offset: int, bins: int, value: int, amount: float = 1.0) -> None:
    slot, sign = _nn_hash_slot(value, bins)
    vec[offset + slot] += float(amount) * sign


def _nn_archetype_index(archetype: str) -> int:
    names = ("unknown", "marnie", "lucario", "alakazam", "archaludon", "dragapult", "crustle", "rocket", "cynthia", "other")
    try:
        return names.index(archetype)
    except ValueError:
        return 9


def _nn_phase_index(obs: Observation) -> int:
    names = ("setup", "early", "mid", "late", "desperate")
    phase = _learned_phase(obs)
    try:
...
def _nn_state_features(obs: Observation, archetype: str, teacher_action: list[int]) -> Any:
    if _np is None:
        return None
    v = _np.zeros(_NN_STATE_DIM, dtype=_np.float32)
    st = obs.current
    me = st.yourIndex
    mine = st.players[me]
    opp = st.players[1 - me]
    mine_field = [p for _, _, p in _field(mine)]
    opp_field = [p for _, _, p in _field(opp)]
    mine_hand = list(mine.hand or [])
    mine_discard = list(mine.discard or [])
    select = obs.select
    scalars = [
        min(1.0, float(st.turn or 0) / 40.0),
        min(1.0, float(st.turnActionCount or 0) / 20.0),
        float(me), float(st.firstPlayer == me), float(st.firstPlayer == 1 - me),
        float(bool(st.energyAttached)), float(bool(st.retreated)), float(bool(st.stadiumPlayed)), float(bool(st.supporterPlayed)),
        float(mine.deckCount or 0) / 60.0, float(mine.handCount or 0) / 15.0, float(len(mine.prize or [])) / 6.0,
        float(len(mine.bench or [])) / 5.0, float(len(mine.active or [])),
        float(opp.deckCount or 0) / 60.0, float(opp.handCount or 0) / 15.0, float(len(opp.prize or [])) / 6.0,
        float(len(opp.bench or [])) / 5.0, float(len(opp.active or [])),
        min(1.0, sum(_energy_count(p) for p in mine_field) / 12.0),
        min(1.0, sum(_energy_count(p) for p in opp_field) / 12.0),
        min(1.0, sum(_damage_on(p) for p in mine_field) / 500.0),
        min(1.0, sum(_damage_on(p) for p in opp_field) / 500.0),
        min(1.0, sum(_ready_to_attack(p) for p in mine_field) / 4.0),
        min(1.0, sum(_ready_to_attack(p) for p in opp_field) / 4.0),
        min(1.0, sum(_has_ability(p.id) for p in mine_field) / 5.0),
        min(1.0, sum(_has_ability(p.id) for p in opp_field) / 5.0),
        float(select.minCount or 0) / 5.0, float(select.maxCount or 0) / 5.0,
        min(1.0, len(select.option or []) / 20.0),
        float(len(teacher_action or [])) / 5.0,
        min(1.0, float(getattr(select, "remainEnergyCost", 0) or 0) / 5.0),
    ]
    v[:len(scalars)] = scalars
    v[32 + _nn_archetype_index(archetype)] = 1.0
    v[42 + _nn_phase_index(obs)] = 1.0
    context = int(select.context)
    v[48 + (context % 16)] = 1.0
    v[64 + (int(select.type) % 8)] = 1.0
    source = _source_id(select)
    _nn_add_hash(v, 72, 8, source, 1.0)
    for p in mine_field:
        _nn_add_hash(v, 80, 24, p.id, 1.0 + 0.15 * _energy_count(p))
    for c in mine_hand:
        _nn_add_hash(v, 80, 24, c.id, 0.45)
    for c in mine_discard:
        _nn_add_hash(v, 80, 24, c.id, 0.12)
    for p in opp_field:
        _nn_add_hash(v, 104, 24, p.id, 1.0 + 0.15 * _energy_count(p))
    return v


def _nn_option_features(obs: Observation, option, archetype: str, teacher_action: list[int], option_index: int) -> Any:
    if _np is None:
        return None
    v = _np.zeros(_NN_OPTION_DIM, dtype=_np.float32)
    typ = int(option.type)
    v[typ % 16] = 1.0
    subject, target = _learned_option_subject(obs, option)
    _nn_add_hash(v, 16, 24, subject, 1.0)
    _nn_add_hash(v, 40, 12, target, 1.0)
    area = int(getattr(option, "area", 0) or 0)
    v[52 + (area % 8)] = 1.0
    player_index = getattr(option, "playerIndex", None)
    me = obs.current.yourIndex
    v[60] = float(player_index is None)
    v[61] = float(player_index == me)
    v[62] = float(player_index == 1 - me)
    v[63] = 0.0  # teacher action is not exposed to the network
    v[64] = min(1.0, float(getattr(option, "number", 0) or 0) / 10.0)
    v[65] = min(1.0, float(getattr(option, "count", 0) or 0) / 10.0)
    v[66] = float(option_index) / max(1.0, float(len(obs.select.option) - 1))
    v[67] = float(subject in OUR_POKEMON)
    data = _card_data(subject)
    if data is not None:
        try:
            v[68 + (int(data.cardType) % 5)] = 1.0
        except Exception:
            pass
        v[73] = min(1.0, float(getattr(data, "hp", 0) or 0) / 350.0)
        v[74] = float(_has_ability(subject))
        v[75] = min(1.0, float(_stage_value(subject)) / 3.0)
        v[76] = min(1.0, float(_min_attack_cost(subject)) / 5.0)
        v[77] = min(1.0, float(_max_attack_damage(subject)) / 350.0)
    v[78] = float(subject == GRIMMSNARL_EX)
    v[79] = float(subject == FROSLASS)
    v[80] = float(subject == MUNKIDORI)
    v[81] = float(subject == DARK_ENERGY)
    v[82] = float(subject in {LILLIES_DETERMINATION, TEAM_ROCKETS_PETREL, JUDGE})
    v[83] = float(subject == BOSSS_ORDERS)
    interaction = (int(obs.select.context) * 1315423911) ^ (typ * 2654435761) ^ (subject * 97) ^ (_source_id(obs.select) * 17) ^ (_nn_archetype_index(archetype) * 53)
    _nn_add_hash(v, 84, 12, interaction, 1.0)
    return v


def _nn_build_features(obs: Observation, teacher_action: list[int]) -> tuple[Any, Any, str]:
    archetype = _update_memory(obs)
    state = _nn_state_features(obs, archetype, teacher_action)
    if state is None:
        return None, None, archetype
    rows = []
    for i, option in enumerate(obs.select.option):
        opt = _nn_option_features(obs, option, archetype, teacher_action, i)
        rows.append(_np.concatenate([state, opt]))
    return state, _np.stack(rows, axis=0) if rows else _np.zeros((0, _NN_INPUT_DIM), dtype=_np.float32), archetype


def _nn_forward(rows, state):
    m = _NN_MODEL
    h1 = _np.maximum(rows @ m["w1"] + m["b1"], 0.0)
    h2 = _np.maximum(h1 @ m["w2"] + m["b2"], 0.0)
    logits = (h2 @ m["wo"] + m["bo"]).reshape(-1)
    ch = _np.maximum(state @ m["cw1"] + m["cb1"], 0.0)
    count_logits = (ch @ m["cwo"] + m["cbo"]).reshape(-1)
    eh = _np.maximum(state @ m["ew1"] + m["eb1"], 0.0)
    expert_logits = (eh @ m["ewo"] + m["ebo"]).reshape(-1)
    return logits, count_logits, expert_logits


def _expert_candidate_actions(obs: Observation) -> tuple[list[list[int]], list[dict[str, Any]]]:
    global _REPLAY_CORRECTION
    base_correction = _REPLAY_CORRECTION
    base_memory = dict(_MEMORY)
    actions: list[list[int]] = []
    memories: list[dict[str, Any]] = []
    try:
        for entries in _EXPERT_CORRECTIONS:
            _MEMORY.clear(); _MEMORY.update(base_memory)
            _REPLAY_CORRECTION = entries
            action = _rule_strategy(obs)
            actions.append(action)
            memories.append(dict(_MEMORY))
    finally:
        _REPLAY_CORRECTION = base_correction
        _MEMORY.clear(); _MEMORY.update(base_memory)
    return actions, memories


def _nn_strategy(obs: Observation, raw_observation: Optional[dict] = None) -> list[int]:
    global _NN_LAST_EXPERT, _NN_LAST_ACTIONS
    # Execute all independent experts on the same observation. They maintain
    # their own internal memories, while the neural policy chooses the action.
    teacher_action = _rule_strategy(obs)
    actions: list[list[int]] = []
    if raw_observation is not None and len(_EXTERNAL_EXPERTS) == 4:
        for expert in _EXTERNAL_EXPERTS[:2]:
            try:
                actions.append(expert.agent(raw_observation))
            except Exception:
        ...
        actions = [teacher_action] * 5
    _NN_LAST_ACTIONS = actions
    if not _NN_MODEL or _np is None or not obs.select.option:
        _NN_LAST_EXPERT = 2
        return teacher_action
    try:
        state, rows, _ = _nn_build_features(obs, teacher_action)
        option_logits, count_logits, expert_logits = _nn_forward(rows, state)
        option_logits = option_logits / max(0.2, float(_NN_META.get("temperature", 1.0)))
        candidate_scores = []
        for j, action in enumerate(actions):
            vals = [float(option_logits[i]) for i in action if isinstance(i, int) and 0 <= i < len(option_logits)]
            option_term = sum(vals) / max(1, len(vals)) if vals else 0.0
            candidate_scores.append(float(expert_logits[j]) + 0.10 * option_term)
        order = sorted(range(5), key=lambda j: (candidate_scores[j], j), reverse=True)
        chosen = order[0]
        margin = candidate_scores[order[0]] - candidate_scores[order[1]]
        confidence = float(_NN_META.get("expert_confidence", 0.08))
        if chosen != 2 and margin < confidence:
            chosen = 2
        mine = obs.current.players[obs.current.yourIndex]
        opponent = obs.current.players[1 - obs.current.yourIndex]
        for j, action in enumerate(actions):
            if any(isinstance(i, int) and 0 <= i < len(obs.select.option) and _immediate_winning_attack(obs, obs.select.option[i], mine, opponent) for i in action):
                chosen = j
                break
        _NN_LAST_EXPERT = chosen
...
def _strategy(obs: Observation, raw_observation: Optional[dict] = None) -> list[int]:
    return _nn_strategy(obs, raw_observation)
...
def agent(observation: dict) -> list[int]:
    if observation.get("select") is None:
        _reset_memory()
        for _expert in _EXTERNAL_EXPERTS:
            try:
                _expert.agent(observation)
            except Exception:
                pass
        return MY_DECK
    try:
        obs = to_observation_class(observation)
        result = _strategy(obs, observation)
