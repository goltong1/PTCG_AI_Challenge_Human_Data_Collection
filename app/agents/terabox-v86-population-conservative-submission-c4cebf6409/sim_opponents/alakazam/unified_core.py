...
class UnifiedController:
    ...
    def __init__(self, base, api, config, deck_catalog, replay_prior=None, opponent_bank=None):
        ...
        self._seen=set(); self._turn_searched=-1; self._native_root_turn=-1; self._loop_sig=None; self._loop_count=0; self._guard_turn=-1; self._guard_count=0; self.stats={'searches':0,'overrides':0,'recognized':Counter(),'opponent_turns':0,'opponent_actions':0,'opponent_errors':0,'native_root_calls':0,'hybrid_searches':0}
    ...
    def reset(self):
        self._seen.clear(); self._turn_searched=-1; self._native_root_turn=-1; self._loop_sig=None; self._loop_count=0; self._guard_turn=-1; self._guard_count=0
    ...
    def loop_break(self,obs,act):
        ...
        if s.turn!=self._guard_turn:
            self._guard_turn=s.turn;self._guard_count=0
        self._guard_count+=1
        if sig==self._loop_sig:self._loop_count+=1
        else:self._loop_sig=sig;self._loop_count=1
        # An ordinary turn never approaches this budget. This only terminates
        # pathological draw/recycle loops without changing normal decisions.
        if self._loop_count>=3 or self._guard_count>=400:
            for i,o in enumerate(obs.select.option):
                if o.type==self.api.OptionType.ATTACK:
                    self._loop_count=0;self._guard_count=0;return [i]
            for i,o in enumerate(obs.select.option):
    ...
    def agent(self,d):
        ...
        try:
            ...
            # Submission safety: a malformed/search-induced action loop must not
            # consume the whole episode. Normal turns stay far below this cap.
            if obs.current is not None and obs.select.context==self.api.SelectContext.MAIN and obs.current.turnActionCount>80:
                for i,o in enumerate(obs.select.option):
                    if o.type==self.api.OptionType.END:return [i]
