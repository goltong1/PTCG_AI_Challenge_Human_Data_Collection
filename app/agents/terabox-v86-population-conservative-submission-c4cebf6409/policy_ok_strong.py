...
# === v22 early Cornerstone completion sequence ===
V22={'corner': 16000, 'latias': 9000, 'prism_pen': 18000, 'direct': 1}
_V22_ROLE=_x_role;_V22_SEARCH=_x_search_score;_V22_ATTACH=_x_attach_score;_V22_TRANSFER=_x_transfer_plan;_V22_MAIN=_x_main

def _v22_ready(obs):return any(p.id==X['CORNER'] and _x_ready_damage(obs,p)>0 for p in _x_board(obs,True))
def _x_role(obs,c,instance=False):
 v=_V22_ROLE(obs,c,instance);cid=int(getattr(c,'id',0) or 0)
 if cid==X['CORNER'] and not _x_has(obs,cid):v+=V22['corner']
 if cid==X['LATIAS'] and not _x_has(obs,cid):v+=V22['latias']
 return v

def _x_search_score(obs,c,selected=None):
 v=_V22_SEARCH(obs,c,selected);cid=int(getattr(c,'id',0) or 0)
 if cid==X['CORNER'] and not _x_has(obs,cid):v+=V22['corner']
 if cid==X['LATIAS'] and not _x_has(obs,cid):v+=V22['latias']
 return v

def _x_attach_score(obs,o):
 v=_V22_ATTACH(obs,o);e=_source(obs,o);t=_target(obs,o)
 if not e or not t:return v
 if int(e.id)==X['PRISM_CARD']:
  if t.id==X['CORNER'] and not _v22_ready(obs):v+=V22['corner']
  elif not _v22_ready(obs):v-=V22['prism_pen']
 if int(e.id)==X['G'] and t.id==X['CORNER'] and not _v22_ready(obs):v+=4500
 return v

def _x_transfer_plan(obs):
 plan=_V22_TRANSFER(obs)
 if _v22_ready(obs):return plan
 # Before Demolish is online, only accept transfers targeting Cornerstone.
 if plan and int(plan[-1])==X['CORNER']:return plan
 return None

def _x_main(obs):
 if V22.get('direct') and not _x_has(obs,X['CORNER']):
  # Tera Orb/Cornerstone are free setup actions and have first-turn time value.
  for cid in (X['TERA_ORB'],X['CORNER']):
   for i,o in enumerate(obs.select.option):
    c=_source(obs,o)
    if o.type==OptionType.PLAY and c and int(c.id)==cid:return [i]
 pl=_x_pl(obs,True);a=pl.active[0] if pl.active else None
 if a and a.id==X['CORNER'] and _x_ready_damage(obs,a)>0:
  ats=[i for i,o in enumerate(obs.select.option) if o.type==OptionType.ATTACK]
  if ats:return [max(ats,key=lambda i:_x_damage(obs,a,obs.select.option[i].attackId))]
 return _V22_MAIN(obs)
