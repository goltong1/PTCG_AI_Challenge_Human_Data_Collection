...
# === v17 Lucario weakness-conversion specialist =============================
_L17_ROLE=_x_role;_L17_SEARCH=_x_search_score;_L17_ATTACH=_x_attach_score
_L17_DAMAGE=_x_damage;_L17_MAIN=_x_main

def _x_matchup(obs):return 'lucario'
def _x_primary(obs):return X['CLEF']
def _x_tech_primary(obs):return X['CLEF']
def _x_secondary(obs):return X['TEAL']

def _x_damage(obs,p,aid):
 v=_L17_DAMAGE(obs,p,aid)
 try:
  oa=_x_pl(obs,False).active[0] if _x_pl(obs,False).active else None
  if p and p.id==X['CLEF'] and int(aid or 0)==371 and oa and oa.id in {333,677,678}:return v*2
 except Exception:pass
 return v

def _x_role(obs,c,instance=False):
 v=_L17_ROLE(obs,c,instance);cid=int(getattr(c,'id',0) or 0)
 if cid==X['CLEF']:v+=13000 if not _x_has(obs,cid) else 1600
 if cid==X['LATIAS'] and not _x_has(obs,cid):v+=7000
 if cid==X['PRISM_CARD'] and _x_has(obs,X['CLEF']):v+=8500
 if cid in {X['PECH'],X['MUNKI'],X['CHIYU']} and not _x_has(obs,cid):v-=4200
 return v

def _x_search_score(obs,c,selected=None):
 v=_L17_SEARCH(obs,c,selected);cid=int(getattr(c,'id',0) or 0)
 if cid==X['CLEF'] and not _x_has(obs,cid):v+=16000
 if cid==X['LATIAS'] and not _x_has(obs,cid):v+=8500
 if cid==X['PRISM_CARD'] and _x_has(obs,X['CLEF']):v+=10000
 return v

def _x_attach_score(obs,o):
 v=_L17_ATTACH(obs,o);e=_source(obs,o);t=_target(obs,o)
 if e and t and t.id==X['CLEF']:
  v+=9500
...
def _x_main(obs):
 ...
 # Deny Riolu evolution whenever the current attacker can take the prize.
 if ready>0 and any(p and p.id in {333,677} and ready>=int(p.hp or 0) for p in (_x_pl(obs,False).bench or [])):
  for i,o in enumerate(opts):
   c=_source(obs,o)
   if o.type==OptionType.PLAY and c and c.id==X['BOSS']:return [i]
 ...
 if clef and _x_ready_damage(obs,clef)>0 and (not a or a is not clef) and _x_has(obs,X['LATIAS']):
  for i,o in enumerate(opts):
   if o.type==OptionType.RETREAT:return [i]
 if a and a.id==X['CLEF'] and _x_ready_damage(obs,a)>0:
  ats=[i for i,o in enumerate(opts) if o.type==OptionType.ATTACK]
  if ats:return [max(ats,key=lambda i:_x_damage(obs,a,opts[i].attackId))]
 return _L17_MAIN(obs)
