...
# === v9 independent specialist candidate ===

_V9R=_x_role;_V9Q=_x_search_score;_V9A=_x_attach_score;_V9M=_x_main

def _x_primary(obs):return X['CORNER']
def _x_secondary(obs):return X['KANGA']
def _x_tech_primary(obs):return X['CORNER']
def _x_role(obs,c,instance=False):
 v=_V9R(obs,c,instance);cid=int(getattr(c,'id',0) or 0)
 if cid==X['CORNER']:v+=9000 if not _x_has(obs,cid) else 1000
 if cid==X['KANGA']:v+=2500 if not _x_has(obs,cid) else 300
 return v

def _x_search_score(obs,c,selected=None):
 v=_V9Q(obs,c,selected);cid=int(getattr(c,'id',0) or 0)
 if cid==X['CORNER'] and not _x_has(obs,cid):v+=12000
 if cid==X['PRISM_CARD'] and _x_has(obs,X['CORNER']):v+=6000
 return v

def _x_attach_score(obs,o):
 v=_V9A(obs,o);e=_source(obs,o);t=_target(obs,o)
 if e and t and t.id==X['CORNER'] and _x_energy_progress(t)>0:v+=8000
 return v
...
def _x_main(obs):
 ...
 ready=next((p for p in pl.bench or [] if p and p.id==X['CORNER'] and _x_ready_damage(obs,p)>0),None)
