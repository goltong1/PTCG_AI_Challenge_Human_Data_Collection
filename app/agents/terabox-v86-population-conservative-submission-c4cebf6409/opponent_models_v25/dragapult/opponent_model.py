...
def _c_src(obs,o):
 ...
 return None
def _fast_backup(observation,chosen):
 try:
  if not isinstance(chosen,list) or len(chosen)!=1 or not observation.get('select'):return chosen
  obs=_api.to_observation_class(observation);name,conf=_controller.recognize(obs)
  if name!='lucario_hariyama_fast':return chosen
  sel=obs.select;opts=list(sel.option);ctx=str(sel.context).split('.')[-1].lower();me=obs.current.players[obs.current.yourIndex]
  f=[p for p in list(me.active)+list(me.bench) if p];line=sum(p.id in {119,120,121} for p in f);dp=sum(p.id==119 for p in f)
  if ctx=='tohand' and line<=1 and dp==1:
   for i,o in enumerate(opts):
    c=_c_src(obs,o)
    if c is not None and int(c.id)==119:return [i]
  if ctx=='main' and int(obs.current.turn)<=3 and line<=1:
   for i,o in enumerate(opts):
    c=_c_src(obs,o)
    if _c_typ(o,'play',_api.OptionType.PLAY) and c is not None and int(c.id)==1086:return [i]
  return chosen
 except Exception:return chosen
_clean_old=agent
def agent(observation:dict)->list[int]:return _fast_backup(observation,_clean_old(observation))
