...
def _c_typ(o,name,enum):return o.type==enum or str(o.type).split('.')[-1].lower()==name
def _c_src(obs,o):
 try:
  c=_controller._source_card(obs,o)
  if c is not None:return c
 except Exception:pass
 try:
  if o.index is not None:return obs.current.players[obs.current.yourIndex].hand[o.index]
 except Exception:pass
 return None
def _c_tgt(obs,o):
 try:
  t=_controller._target(obs,o)
  if t is not None:return t
 except Exception:pass
 try:
  pl=obs.current.players[obs.current.yourIndex];ar=int(o.inPlayArea) if o.inPlayArea is not None else -1
  return pl.active[o.inPlayIndex] if ar==4 else pl.bench[o.inPlayIndex] if ar==5 else None
 except Exception:return None
def _alak_line(observation,chosen):
 try:
  if not isinstance(chosen,list) or len(chosen)!=1 or not observation.get('select'):return chosen
  obs=_api.to_observation_class(observation);name,conf=_controller.recognize(obs)
  if name not in {'alakazam','alakazam_hilda_control'}:return chosen
  sel=obs.select;opts=list(sel.option);ctx=str(sel.context).split('.')[-1].lower();me=obs.current.players[obs.current.yourIndex]
  f=[p for p in list(me.active)+list(me.bench) if p];has=any(p.id in {333,677,678} for p in f)
  if ctx=='tohand' and not has:
   for pref in (677,333):
    for i,o in enumerate(opts):
     c=_c_src(obs,o)
     if c is not None and int(c.id)==pref:return [i]
  if ctx=='main' and int(obs.current.turn)<=7:
   if not has:
    for wanted in (1086,1152,1102):
     for i,o in enumerate(opts):
      c=_c_src(obs,o)
      if _c_typ(o,'play',_api.OptionType.PLAY) and c is not None and int(c.id)==wanted:return [i]
    for i,o in enumerate(opts):
     c=_c_src(obs,o)
     if _c_typ(o,'play',_api.OptionType.PLAY) and c is not None and int(c.id) in {333,677}:return [i]
   att=[]
   for i,o in enumerate(opts):
    if _c_typ(o,'attach',_api.OptionType.ATTACH):
     t=_c_tgt(obs,o)
     if t is not None and int(t.id) in {333,677,678}:att.append(i)
   ci=chosen[0]
   if att and _c_typ(opts[ci],'attach',_api.OptionType.ATTACH):
    t=_c_tgt(obs,opts[ci])
    if t is None or int(t.id) not in {333,677,678}:return [att[0]]
  return chosen
 except Exception:return chosen
_clean_old=agent
def agent(observation:dict)->list[int]:return _alak_line(observation,_clean_old(observation))
