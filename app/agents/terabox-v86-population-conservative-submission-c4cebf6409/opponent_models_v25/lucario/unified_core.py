...
class UnifiedController:
    ...
    def recognize(self,obs):
        ...
        for name,ds in self._deck_sets.items():
            ...
            evidence=(self.cfg.get('recognition_evidence') or {}).get(name,{})
            ev=sum(float(evidence.get(str(x),0.0)) for x in self._seen)
            scores.append((hit-miss+ev*float(self.cfg.get('recognition_evidence_scale',0.0)),name))
    ...
    def hidden(self,obs,name,extra_rot=0):
        s=obs.current;me=s.yourIndex;a=s.players[me];b=s.players[1-me]
        own=self._remaining(self.my_deck,self._known(a,True),a.deckCount+len(a.prize),s.turn*3+me+int(extra_rot)*7)
        yp=own[:len(a.prize)];yd=own[len(a.prize):]
        odfull=self.catalog[name]
        rem=self._remaining(odfull,self._known(b,False),b.deckCount+len(b.prize)+b.handCount,s.turn*5+me+int(extra_rot)*11)
    ...
    def search(self,obs,base,name,conf):
        ...
        samples=max(1,int(self._param(name,'robust_hidden_samples',self.cfg.get('robust_hidden_samples',1))))
        try:
            for sample in range(samples):
                try:
                    h=self.hidden(obs,name,sample)
        ...
        vals={i:sum(vs)/len(vs) for i,vs in sample_vals.items() if vs}
        if not vals:return base
        bi=base[0] if base and len(base)==1 else None
        if bi not in vals:return base
        best=max(vals,key=vals.get);margin=float(self._param(name,'override_margin',100000))
        gains=[a-b for a,b in zip(sample_vals.get(best,[]),sample_vals.get(bi,[]))]
        frac=float(self._param(name,'robust_positive_fraction',self.cfg.get('robust_positive_fraction',0.5)))
        catastrophe=float(self._param(name,'robust_catastrophe_margin',self.cfg.get('robust_catastrophe_margin',margin*0.5)))
        robust_ok=(not gains) or (sum(g>=0 for g in gains)>=max(1,int(math.ceil(len(gains)*frac))) and min(gains)>=-catastrophe)
        if best!=bi and vals[best]>=vals[bi]+margin and robust_ok:
            self.stats['overrides']+=1;return [best]
        return base
    ...
    def agent(self,d):
        ...
        try:
            ...
            if name=='marnie':
                sig=set(self.cfg.get('marnie_signature_cards') or [646,647,648,1219,1259])
                if not (set(self._seen) & sig):
                    name=None; conf=0.0
