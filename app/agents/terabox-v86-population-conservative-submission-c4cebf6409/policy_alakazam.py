...
TB={
 ...
 'CHIYU':31,'PECH':898,'PECH_EX':141,'BUG':1094,'LILLIE':1227,'AREA':1250,
...
# === v18 replay contrast residual ===
V18={'name': 'teal', 'primary': 'TEAL', 'infra': 5000, 'latias': 6500, 'kanga': 2500, 'attack_turn': 3, 'tech_pen': 5000}
_V18_ROLE=_x_role;_V18_SEARCH=_x_search_score;_V18_ATTACH=_x_attach_score;_V18_MAIN=_x_main;_V18_PRIMARY=_x_primary;_V18_TECH=_x_tech_primary

def _v18_core(obs):
 return _x_has(obs,X['LATIAS']) and (_x_has(obs,X['KANGA']) or _x_count(obs,X['TEAL'])>=1)

def _x_primary(obs):
 mode=V18.get('primary')
 if mode=='TEAL':return X['TEAL']
 if mode=='CORNER':return X['CORNER']
 if mode=='CORE_CHIYU':return X['CHIYU'] if _v18_core(obs) else X['TEAL']
 if mode=='KANGA_CLEF':
  k=next((p for p in _x_board(obs,True) if p.id==X['KANGA']),None)
  return X['KANGA'] if not _v18_core(obs) or (k is not None and _x_ready_damage(obs,k)>0) else X['CLEF']
 if mode=='KANGA_TEAL':
  k=next((p for p in _x_board(obs,True) if p.id==X['KANGA']),None)
  return X['KANGA'] if not _v18_core(obs) or (k is not None and _x_ready_damage(obs,k)>0) else X['TEAL']
 return _V18_PRIMARY(obs)

def _x_tech_primary(obs):
 mode=V18.get('primary')
 if mode=='TEAL':return X['TEAL']
 if mode=='CORNER':return X['CORNER']
 if mode=='CORE_CHIYU':return X['CHIYU']
 if mode=='KANGA_CLEF':return X['CLEF']
 if mode=='KANGA_TEAL':return X['TEAL']
 return _V18_TECH(obs)

def _x_role(obs,c,instance=False):
 v=_V18_ROLE(obs,c,instance);cid=int(getattr(c,'id',0) or 0);turn=int(obs.current.turn or 0)
 if cid==X['LATIAS'] and not _x_has(obs,cid):v+=V18.get('latias',0)
 if cid==X['KANGA'] and not _x_has(obs,cid):v+=V18.get('kanga',0)
 if cid==X['TEAL'] and _x_count(obs,cid)<V18.get('teal_cap',2):v+=V18.get('infra',0)
 if cid==X['CORNER'] and not _x_has(obs,cid):v+=V18.get('corner',0)
 if cid==X['CHIYU'] and not _x_has(obs,cid):v+=V18.get('tech',0) if _v18_core(obs) else -abs(V18.get('tech_pen',3000))
 if cid in {X['CORNER'],X['CLEF']} and V18.get('primary')=='TEAL' and turn<=6:v-=V18.get('tech_pen',0)
 cap=V18.get('bench_cap')
 if cap is not None and len(_x_board(obs,True))>=cap and cid not in {X['LATIAS'],X['KANGA'],X['TEAL'],_x_primary(obs)}:v-=10000
 return v

def _x_search_score(obs,c,selected=None):
 v=_V18_SEARCH(obs,c,selected);cid=int(getattr(c,'id',0) or 0);turn=int(obs.current.turn or 0)
 if cid==X['LATIAS'] and not _x_has(obs,cid):v+=V18.get('latias',0)
 if cid==X['KANGA'] and not _x_has(obs,cid):v+=V18.get('kanga',0)
 if cid==X['TEAL'] and _x_count(obs,cid)<V18.get('teal_cap',2):v+=V18.get('infra',0)
 if cid==X['CORNER'] and not _x_has(obs,cid):v+=V18.get('corner',0)
...
def _x_main(obs):
 # Free abilities remain under the learned base policy; this residual only changes
 # ordering among setup/cycle/attack actions.
 pl=_x_pl(obs,True);active=pl.active[0] if pl.active else None;turn=int(obs.current.turn or 0)
 # Resolve directly playable infrastructure before a shuffle cycle.
 for cid in (X['LATIAS'],X['KANGA'],X['TEAL']):
  need=(cid==X['LATIAS'] and not _x_has(obs,cid)) or (cid==X['KANGA'] and not _x_has(obs,cid)) or (cid==X['TEAL'] and _x_count(obs,cid)<V18.get('teal_cap',2))
  if need:
   for i,o in enumerate(obs.select.option):
    c=_source(obs,o)
    if o.type==OptionType.PLAY and c and int(c.id)==cid:return [i]
 # Meaningful ready attacks occur before optional search/cycle once setup window passed.
 ready=_x_ready_damage(obs,active)
 if ready>0 and turn>=V18.get('attack_turn',3):
 ...
 out=_V18_MAIN(obs)
 # If base selected a low-EV Lillie after an executable attack became available,
 # keep the existing policy unless the expected redraw gain is genuinely positive.
 try:
  if out and len(out)==1:
   o=obs.select.option[out[0]];c=_source(obs,o)
   if o.type==OptionType.PLAY and c and c.id==X['LILLIE'] and _x_lillie_gain(obs)<V18.get('cycle_floor',500):
    attacks=[i for i,z in enumerate(obs.select.option) if z.type==OptionType.ATTACK]
    if attacks:return [max(attacks,key=lambda i:_x_damage(obs,active,obs.select.option[i].attackId))]
 except Exception:pass
 return out

# === v19 evolution-denial tempo residual ===================================
_V19_MAIN=_x_main

def _x_main(obs):
 pl=_x_pl(obs,True);a=pl.active[0] if pl.active else None;ready=_x_ready_damage(obs,a)
 if ready>0:
  best=None
  for j,p in enumerate(_x_pl(obs,False).bench or []):
   if not p or ready<int(p.hp or 0):continue
   cd=CARDS.get(p.id);pr=3 if cd and cd.megaEx else 2 if cd and cd.ex else 1
   denial=8500 if p.id in {741,742} else 4200 if p.id in {743,245} else 0
   z=(denial+pr*1800+max(0,160-int(p.hp or 0)),j)
   if best is None or z>best:best=z
  if best and best[0]>=5000:
   for i,o in enumerate(obs.select.option):
    c=_source(obs,o)
    if o.type==OptionType.PLAY and c and c.id==X['BOSS']:return [i]
 return _V19_MAIN(obs)
