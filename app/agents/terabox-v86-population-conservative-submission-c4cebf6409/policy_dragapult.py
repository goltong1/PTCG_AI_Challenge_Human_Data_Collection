...
# === v17 Dragapult prize-race residual ======================================
# Kangaskhan remains the first stable attacker.  Clefairy is the conversion
# attacker: with enough combined Bench count its Psychic weakness hit can take
# Dragapult ex in one attack.  Only high-HP Bench bodies are added for that
# threshold; small utility Pokémon remain liabilities to Phantom Dive.
_V17_ROLE=_x_role
_V17_SEARCH=_x_search_score
_V17_ATTACH=_x_attach_score
_V17_MAIN=_x_main

def _v17_core(obs):
    return _x_has(obs,X['KANGA']) and _x_has(obs,X['LATIAS']) and _x_count(obs,X['TEAL'])>=1

def _v17_safe_body(cid):
    cd=CARDS.get(int(cid or 0))
    return bool(cd and cd.cardType==CardType.POKEMON and cd.basic and int(cd.hp or 0)>=190)

def _x_role(obs,c,instance=False):
    v=_V17_ROLE(obs,c,instance);cid=int(getattr(c,'id',0) or 0);turn=int(obs.current.turn or 0)
    if cid==X['CLEF']:
        v+=9000 if _v17_core(obs) and not _x_has(obs,cid) else 2600 if not _x_has(obs,cid) else 700
    if cid==X['PRISM_CARD'] and _x_has(obs,X['CLEF']):v+=6500
    if cid in {X['PECH'],X['MUNKI'],X['CHIYU']} and turn<=7:v-=5200
    if cid==X['MEOWTH'] and turn>=3:v-=2400
    if _x_has(obs,X['CLEF']) and _v17_safe_body(cid) and not _x_has(obs,cid):v+=1600
    return v

def _x_search_score(obs,c,selected=None):
    v=_V17_SEARCH(obs,c,selected);cid=int(getattr(c,'id',0) or 0);turn=int(obs.current.turn or 0)
    if cid==X['CLEF'] and not _x_has(obs,cid):v+=12500 if _v17_core(obs) else 3500
    if cid==X['PRISM_CARD'] and _x_has(obs,X['CLEF']):v+=9000
    if cid in {X['PECH'],X['MUNKI'],X['CHIYU']} and turn<=7:v-=7000
    if cid==X['MEOWTH'] and turn>=3:v-=3500
    return v

def _x_attach_score(obs,o):
    v=_V17_ATTACH(obs,o);e=_source(obs,o);t=_target(obs,o)
    if not e or not t:return v
    if t.id==X['CLEF']:
        v+=8500
        if int(e.id)==X['PRISM_CARD']:v+=8500
    elif int(e.id)==X['PRISM_CARD'] and _x_has(obs,X['CLEF']) and _x_energy_progress(next((p for p in _x_board(obs,True) if p.id==X['CLEF']),t))>0:
        v-=9000
    return v

def _v17_boss_ko(obs,damage):
    best=None
    for i,p in enumerate(_x_pl(obs,False).bench or []):
        if not p or damage<=0 or damage<int(p.hp or 0):continue
        cd=CARDS.get(p.id);pr=3 if cd and cd.megaEx else 2 if cd and cd.ex else 1
        denial=9000 if p.id in {119,120,235} else 5000 if p.id==121 else 0
        z=(denial+pr*1800+max(0,180-int(p.hp or 0)),i)
        if best is None or z>best:best=z
    return best

def _x_main(obs):
    pl=_x_pl(obs,True);a=pl.active[0] if pl.active else None;opts=obs.select.option
    ready=_x_ready_damage(obs,a)
    # Convert a guaranteed evolution denial before taking a lower-value Active hit.
    ko=_v17_boss_ko(obs,ready)
    if ko:
        for i,o in enumerate(opts):
            c=_source(obs,o)
            if o.type==OptionType.PLAY and c and c.id==X['BOSS']:return [i]
    # Once Clefairy is ready, add a safe body only when the extra 40 weakness
    # damage crosses the current Active's KO threshold.
    clef=next((p for p in _x_board(obs,True) if p.id==X['CLEF']),None)
    oa=_x_pl(obs,False).active[0] if _x_pl(obs,False).active else None
    if clef and _x_ready_damage(obs,clef)>0 and oa:
        now=_x_ready_damage(obs,clef)
        if now<int(oa.hp or 0)<=now+40:
            safe=[]
            for i,o in enumerate(opts):
                c=_source(obs,o)
                if o.type==OptionType.PLAY and c and _v17_safe_body(c.id):safe.append((_x_role(obs,c),i))
            if safe:return [max(safe)[1]]
    # A prepared Clefairy must become Active before more hand cycling.
    if clef and _x_ready_damage(obs,clef)>0 and (not a or a is not clef) and _x_has(obs,X['LATIAS']):
        for i,o in enumerate(opts):
            if o.type==OptionType.RETREAT:return [i]
    out=_V17_MAIN(obs)
    # With an executable attack, permit free abilities and Bug Catching Set,
    # but suppress redundant search/cycle actions that delay the prize race.
    if ready>0 and out:
        try:
            chosen=opts[out[0]];c=_source(obs,chosen)
            productive=(chosen.type==OptionType.ABILITY or
                        (chosen.type==OptionType.PLAY and c and c.id==X['BUG']))
            if not productive:
                attacks=[i for i,o in enumerate(opts) if o.type==OptionType.ATTACK]
