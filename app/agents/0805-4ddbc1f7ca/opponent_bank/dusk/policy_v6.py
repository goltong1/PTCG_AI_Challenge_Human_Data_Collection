...
def agent(obs_dict: dict) -> list[int]:
    ...
    target_priority_ids = {'dragapult': {Dreepy: 7000, Drakloak: 10000, Munkidori: 8000}, 'lucario': {677:8500,673:6500,675:4500,676:4500}, 'alakazam': {741:7500,742:10500}, 'marnie': {646:7600,647:10500,648:12000,104:11800,860:7000,Munkidori:11200}, 'starmie': {1030:8500,860:7500,Munkidori:7000}, 'crustle': {344:12000}}.get(opponent_profile,{})
    ...
    stall_caps = {'dragapult': 4 if mirror_profile == 1 else 3, 'lucario': 3,
                  'alakazam': 2, 'marnie': 4, 'starmie': 2, 'crustle': 1,
                  'rocket': 1, 'unknown': 1}
    ...
    def _dusk_closeout_plan(blast: int):
        ...
        for area, group in ((AreaType.ACTIVE, op_state.active), (AreaType.BENCH, op_state.bench)):
            for target in group:
                if target is None or no_damage_counter(target):
                    continue
                ...
                if blast == 50:
                    ...
                    go = wins or grim_break or direct_finish
                    bonus = 260000 if grim_break else (160000 if direct_finish else 0)
    ...
    grim_noir_route = bool(is_marnie_match and active_target is not None and active_target.id == 648
        and ((active_phantom_ready and active_target.hp <= 330) or active_target.hp <= 130)
        and len(op_state.prize) >= 2)
    dusclops_any = any(p.id == Dusclops for p in own_field)
    search_dusknoir_now = bool(grim_noir_route and dusclops_any and deck_counts[Dusknoir] > 0 and (poke_pad_in_hand or dawn_in_hand))
    preserve_dusclops_for_noir = bool(grim_noir_route and dusclops_any and (dusknoir_in_hand or search_dusknoir_now))
