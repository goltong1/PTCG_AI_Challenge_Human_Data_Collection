...
ARCHETYPE_IDS = {
    ...
    # Preserve v82 Crustle recognition, including its Mega Kangaskhan clue.
    # A unique Ogerpon clue takes precedence below when the list is Tera Box.
    "crustle": {344, 345, 756},
    "lucario": {673, 674, 675, 676, 677, 678},
    # Public Tera Box signatures that are unique inside the supplied league.
    # Do not use Clefairy (272) or Mega Kangaskhan (756) alone: those cards are
    # shared with Alakazam and Crustle, respectively.
    "terabox": {96, 108},
...
def _recognize_from_ids(ids: Iterable[int]) -> str:
    ids = set(ids)
    # Teal Mask and Wellspring are the two supplied-league clues that uniquely
    # identify Tera Box.  Check them before the shared Kangaskhan clue so a
    # confirmed Tera list can replace an earlier conservative Crustle guess.
    if ids & ARCHETYPE_IDS["terabox"]:
        return "terabox"
    # Explicit IDs first.
    order = ("crustle", "dragapult", "lucario", "alakazam", "rocket", "terabox", "marnie")
    ...
    if "team rocket" in names:
        return "rocket"
    if "teal mask ogerpon" in names or "wellspring mask ogerpon" in names:
        return "terabox"
...
def _make_plan(obs: Observation, archetype: str) -> Plan:
    ...
    # Crustle cannot be damaged by Grimmsnarl ex. Two Froslass plus three
    # charged Munkidori are the replay-proven fixed-list counter route.
    if archetype == "crustle" and (344 in _MEMORY["revealed"] or 345 in _MEMORY["revealed"]):
        # V75: retain one Grimmsnarl ex as the durable damage reservoir and
        # reserve the remaining field slots for the non-ex counter engines
        # that can damage Crustle through its Pokemon-ex immunity.
        desired_grimms = 1
        desired_froslass = 2
        desired_munkis = 3
    ...
    if archetype == "archaludon":
        ...
        # Loss-sequence audit linked early Froslass development to losses,
        # while wins consistently established a second Grimmsnarl attacker.
        # Reserve the Bench and evolution tempo for that Stage-2 line.
        if {677, 678} & set(_MEMORY["revealed"]):
            desired_froslass = 0
    elif archetype == "marnie":
        # v126: Froslass is a conditional mirror engine, not an unconditional slot.
        # Prepare it early, but complete it only when the opponent has multiple
        # Ability Pokemon and at least one charged Munkidori can convert our own
        # checkup damage with Adrena-Brain.
        bench_snorunt = any(p is not None and p.id == SNORUNT for p in (mine.bench or []))
        desired_froslass = 1 if (bench_snorunt and opponent_abilities >= 2 and len(charged_munkis) >= 1) else 0
        if desired_froslass and field[FROSLASS] == 0:
            desired_munkis = min(desired_munkis, 2)

    elif archetype == "terabox":
        # Cross-league v76: Tera Box exposes several Ability Pokemon at once.
        # Froslass converts each checkup into distributed damage that Munkidori
        # can consolidate. Keep two real attackers and do not overfill support
        # before the first Grimmsnarl is established.
        desired_grimms = 2
        desired_froslass = 2 if opponent_abilities >= 3 else 1
        desired_munkis = 2 if field[GRIMMSNARL_EX] < 1 else 3
...
def _choose_poffin_set(scores: list[int], obs: Observation, mine, field: Counter,
                       hand: Counter, plan: Plan) -> Optional[list[int]]:
    """Joint marginal-utility selection for multi-target Poffin."""
    select=obs.select
    if select.context != SelectContext.TO_BENCH or _source_id(select) != BUDDY_BUDDY_POFFIN:return None
    me=obs.current.yourIndex; cards=[]
    for i,o in enumerate(select.option):
        if o.type != OptionType.CARD:continue
        c=_get_card(obs,o.area,o.index,o.playerIndex if o.playerIndex is not None else me)
        if c is not None:cards.append((i,c.id))
    if not cards:return []
    from itertools import combinations
    mn=int(select.minCount or 0);mx=min(int(select.maxCount or 0),len(cards))
    line=field[IMPIDIMP]+field[MORGREM]+field[GRIMMSNARL_EX]
    ep=ALAKAZAM_ALT in set(_MEMORY['revealed'])
    sn_target=plan.desired_froslass if ep else max(1,plan.desired_froslass)
    ld=max(0,plan.desired_grimms-line);sd=max(0,sn_target-(field[SNORUNT]+field[FROSLASS]))
    f_ready=hand[FROSLASS]>0 and field[FROSLASS]==0
    best=None;bestkey=None
    for r in range(mn,mx+1):
        for combo in combinations(cards,r):
            idx=[x[0] for x in combo];ids=[x[1] for x in combo]
            if r and all(scores[i]<0 for i in idx):continue
            ni=ids.count(IMPIDIMP);ns=ids.count(SNORUNT)
            v=sum(max(-5000,scores[i]) for i in idx)
            v+=min(ni,ld)*18000+min(ns,sd)*22000
            if f_ready and sd>0 and ns:v+=18000
            v-=max(0,ni-ld)*30000+max(0,ns-sd)*30000
            if ld>0 and sd>0 and ni and ns:v+=9000
            if r==0:v=0
            key=(v,-sum(idx))
            if bestkey is None or key>bestkey:bestkey=key;best=idx
    return sorted(best or [])


def _apply_engine_integrity_guard(scores: list[int], obs: Observation, mine, opponent,
                                  field: Counter, hand: Counter, plan: Plan) -> None:
    """Complete a planned, mature Froslass line before shuffle/draw sequencing."""
    if not scores or obs.select is None or obs.select.context != SelectContext.MAIN:return
    if plan.desired_froslass <= field[FROSLASS]:return
    me=obs.current.yourIndex;legal=[]
    for i,o in enumerate(obs.select.option):
        if o.type != OptionType.EVOLVE:continue
        evo=_get_card(obs,o.area,o.index,me);target=_get_card(obs,o.inPlayArea,o.inPlayIndex,me)
        if evo is not None and target is not None and evo.id==FROSLASS and target.id==SNORUNT and not getattr(target,'appearThisTurn',False):legal.append(i)
    if legal:
        b=max(legal,key=lambda i:(scores[i],-i));scores[b]=max(scores)+970000
...
def _main_score(obs: Observation, option, mine, opponent, field: Counter, hand: Counter,
    ...
    if typ == OptionType.EVOLVE:
        ...
        if evo.id == FROSLASS:
            if field[FROSLASS] >= plan.desired_froslass:
                return NEG
            if plan.archetype == "crustle":
                return 45000 if field[FROSLASS] == 0 else 41000
    ...
    if typ == OptionType.RETREAT:
        if active is None:
            return NEG
        # Crustle prevents Grimmsnarl ex's damage. Once a Munkidori can use
        # Mind Bend, stop spending turns on a blocked ex attack and convert
        # the damaged Grimmsnarl into an Adrena-Brain reservoir instead.
        if (plan.phase == "COUNTER_CONTROL" and active.id == GRIMMSNARL_EX
                and op_active is not None and op_active.id == 345
                and any(p.id == MUNKIDORI and _ready_to_attack(p) for p in (mine.bench or []))):
            return 56000
...
def _card_option_score(obs: Observation, option, mine, opponent, field: Counter, hand: Counter,
    ...
    if context in {SelectContext.SWITCH, SelectContext.TO_ACTIVE}:
        ...
        if (plan.phase == "COUNTER_CONTROL" and cid == MUNKIDORI
                and _ready_to_attack(card)
                and opponent.active and opponent.active[0].id == 345):
            return 56000 + card.hp
...
def _apply_drag_attack_finish(scores: list[int], obs: Observation, mine, archetype: str) -> None:
    """Do not end a Dragapult turn while a legal, meaningful attack exists."""
    if archetype != "dragapult" or obs.select.context != SelectContext.MAIN or not scores:
        return
    top = max(range(len(scores)), key=lambda i: (scores[i], -i))
    if obs.select.option[top].type != OptionType.END or getattr(mine, "confused", False):
        return
    attacks = [
        i for i, option in enumerate(obs.select.option)
        if option.type == OptionType.ATTACK and scores[i] > NEG // 2
    ]
    if attacks:
        best = max(attacks, key=lambda i: (scores[i], -i))
        scores[best] = scores[top] + 930000
...
def _apply_temporal_sequence_guard(scores: list[int], obs: Observation, mine, opponent,
                                   field: Counter, hand: Counter, plan: Plan, source: int) -> None:
    """Protect obvious one-to-three-turn progress without using the weak v62 critic.

    Replay 91098392 opened without an Impidimp, searched a second Munkidori,
    and never established the Stage-2 engine.  Replay 91097501 later left a
    zero-Energy Snorunt Active for many turns while two ready Grimmsnarl sat on
    the Bench.  The linear temporal critic was too noisy to promote globally,
    so this guard transfers only those invariant sequence facts:

    * obtain an evolution-line Basic before redundant support Pokemon;
    * pay a stranded Active's retreat cost before attaching elsewhere;
    * never end a turn while a legal attack or immediately useful evolution
      is the only remaining progress.

    It runs after learned/residual scores and changes nothing when the normal
    policy already chooses a productive continuation.
    """
    if not scores or obs.select is None:
        return
    select = obs.select
    state = obs.current
    me = state.yourIndex
    ...
    def card_for(option):
        return _get_card(obs, option.area, option.index,
                         option.playerIndex if option.playerIndex is not None else me)

    # The supplied loss replays are Mega Abomasnow games.  Keep that signal
    # local to this guard instead of changing the deck's global archetype plan.
    revealed = set(_MEMORY["revealed"])
    replay_abomasnow = bool({722, 723} & revealed)
    lucario_core = plan.archetype == "lucario" and bool({677, 678} & revealed)
    temporal_target = plan.archetype == "alakazam" or lucario_core
    if not temporal_target and not replay_abomasnow:
        return

    # A Pokemon search with no Marnie line in play or hand must establish the
    # deck's engine.  Learned residuals previously overturned this by a small
    # amount and selected a redundant Munkidori.
    if (select.context == SelectContext.TO_HAND and source == POKE_PAD
            and temporal_target):
        line_on_board = field[IMPIDIMP] + field[MORGREM] + field[GRIMMSNARL_EX]
        # A Morgrem without an Impidimp is not a live line.
        if line_on_board == 0 and hand[IMPIDIMP] == 0:
            impidimp = [i for i, option in enumerate(select.option)
                        if option.type == OptionType.CARD
                        and getattr(card_for(option), "id", None) == IMPIDIMP]
            if impidimp:
                best = max(impidimp, key=lambda i: (scores[i], -i))
                scores[best] = max(scores) + 900000
        return
    ...
    ready_bench = [p for p in (mine.bench or []) if p is not None and _ready_to_attack(p)]
    retreat_indices = [i for i, option in enumerate(select.option) if option.type == OptionType.RETREAT]

    # Exact two-step escape: attach to the stranded Active now, retreat in the
    # following decision, then attack with the already-ready Bench Pokemon.
    if not replay_abomasnow and ready_bench and not _ready_to_attack(active):
        if retreat_indices:
            best = max(retreat_indices, key=lambda i: (scores[i], -i))
            scores[best] = max(scores) + 950000
            return
        if not state.energyAttached and _energy_count(active) < _retreat_cost(active):
            escape_attach = []
            for i, option in enumerate(select.option):
                if option.type != OptionType.ATTACH:
                    continue
                target = _get_card(obs, option.inPlayArea, option.inPlayIndex, me)
                energy = _get_card(obs, option.area, option.index, me)
                if (target is not None and target.serial == active.serial
                        and energy is not None
                        and energy.id in {DARK_ENERGY, PRISM_ENERGY, FIRE_ENERGY}
                        and _energy_count(active) + 1 >= _retreat_cost(active)):
                    escape_attach.append(i)
            if escape_attach:
                best = max(escape_attach, key=lambda i: (scores[i], -i))
                scores[best] = max(scores) + 940000
                return

    top = max(range(len(scores)), key=lambda i: (scores[i], -i))
    if select.option[top].type != OptionType.END:
        return

    # If END is currently preferred, rescue only concrete progress.  Attack
    # scoring already understands prevention effects and immediate prize value.
    attacks = [i for i, option in enumerate(select.option)
               if option.type == OptionType.ATTACK and scores[i] > NEG // 2]
    if attacks:
        best = max(attacks, key=lambda i: (scores[i], -i))
        scores[best] = scores[top] + 930000
    ...
    for i, option in enumerate(select.option):
        if option.type != OptionType.EVOLVE:
            continue
        evo = _get_card(obs, option.area, option.index, me)
        target = _get_card(obs, option.inPlayArea, option.inPlayIndex, me)
        if evo is None or target is None:
            continue
        useful = (
            (evo.id == GRIMMSNARL_EX and field[GRIMMSNARL_EX] < 3)
            or (evo.id == MORGREM and field[GRIMMSNARL_EX] < 3)
            or (evo.id == FROSLASS and target.serial == active.serial)
            or (evo.id == FROSLASS and field[FROSLASS] < plan.desired_froslass)
        )
        if useful:
            evolutions.append(i)
    if evolutions:
        best = max(evolutions, key=lambda i: (scores[i], -i))
        scores[best] = scores[top] + 910000
...
def _strategy(obs: Observation) -> list[int]:
    ...
    _apply_temporal_sequence_guard(scores, obs, mine, opponent, field, hand, plan, source)
    _apply_drag_attack_finish(scores, obs, mine, archetype)
    _apply_engine_integrity_guard(scores, obs, mine, opponent, field, hand, plan)

    forced_max = _forced_punk_up_count(obs, mine, field, plan, source)
    poffin_set = _choose_poffin_set(scores, obs, mine, field, hand, plan)
    result = poffin_set if poffin_set is not None else _choose(scores, select, forced_max)
    if result:
