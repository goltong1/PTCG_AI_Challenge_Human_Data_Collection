...
class C:
    ...
    DURALUDON=169; ARCHALUDON=190; CINDERACE=666
...
class StrategyPlanner:
    ...
    def recognize(self):
        ...
        for a in ('crustle','dragapult','alakazam','archaludon','marnie','lucario'):
    ...
    def build(self,obs):
        ...
        if a=='crustle' and ({C.DWEBBLE,C.CRUSTLE}&self.revealed):
            ...
            exn=sum(1 for q in self._field(op) if CARD[q.id].ex or CARD[q.id].megaEx)
            if exn>=2:
...
class StrategyArbiter:
    ...
    def __init__(self):
        self.reset()
    def reset(self):
        self.finish_turn=-1;self.finish_target_serial=-1;self.finish_damage=0;self.alak_ppp_turn=-1;self.alak_ppp_used=0;_CLOSE.reset()
    ...
    def choose(self,obs_dict,base):
        ...
        tree=None
        try:
            if obs.select.context==SelectContext.MAIN: tree=_MT.decide(obs,plan.archetype)
        ...
        # CRUSTLE DECK-RUNWAY RECYCLE -----------------------------------------
        # In long Mysterious Rock Inn games, a 0-5 card deck and a large hand are
        # common. Judge/Lillie can recycle that hand back into the deck and buy full
        # turns of runway. This is deliberately guarded: never replace an attack,
        # retreat, evolution, attachment, Ability, Boss, Hilda, or other live tactical
        # line. It may only replace legacy END or low-runway non-terminal consumption,
        # and only when the resolved supporter increases deck size by >=5 cards.
        # Same-turn mates have already been claimed by the closeout planner above.
        if plan.archetype=='crustle' and ctx==SelectContext.MAIN and not obs.current.supporterPlayed:
            recycle_base=False
            if base and len(base)==1 and 0<=base[0]<len(opts):
                bo=opts[base[0]]
                if bo.type==OptionType.END:
                    recycle_base=True
                elif bo.type==OptionType.PLAY:
                    bc=_card(obs,AreaType.HAND,bo.index,me)
                    recycle_base=bool(bc is not None and bc.id in {C.PPP,C.POKE_PAD,C.GONG,C.POFFIN,C.XEROSIC})
            deck_n=int(getattr(mine,'deckCount',0) or 0); hand_n=len(mine.hand or [])
            if recycle_base and deck_n<=5 and hand_n>=5:
                recycle=[]
                ...
                if recycle:
                    ...
                    _PLANNER.note_override('crustle:deck_runway_recycle_judge' if cid==C.JUDGE else 'crustle:deck_runway_recycle_lillie')
                    return [j]
            ...
            if not (CARD[a0.id].ex or CARD[a0.id].megaEx):
                ...
                if base_retreat:
                    ...
                    for q in (mine.bench or []):
                        ...
                        rock=any(getattr(e,'id',None)==C.ROCK_F for e in (q.energies or []))
                        aura_net=100+30*ppp
        ...
        if plan.strategy=='HAND_DENIAL':
            if ctx==SelectContext.MAIN and not obs.current.energyAttached:
                ...
                for i,o in enumerate(opts):
                    ...
                    if c is not None and p is not None and c.id==C.ROCK_F and _is_lucario_line(p):rock.append(i)
                if rock:
                    def rs(i):
            ...
            if ctx==SelectContext.MAIN and not obs.current.supporterPlayed and op.handCount>=5:
                ...
                protected=bool(mine.active and mine.active[0].id==C.LUCARIO and any(getattr(e,'id',None)==C.ROCK_F for e in (mine.active[0].energies or [])))
            ...
            if ctx==SelectContext.TO_HAND and not obs.current.energyAttached:
                ...
                if rock and any(_is_lucario_line(p) for p in _field(mine)):
...
def agent(obs_dict:dict)->list[int]:
    ...
    if obs_dict.get('select') is None and 'current' not in obs_dict:
        _PLANNER.reset();_MT.reset();_ARBITER.reset()
    base=_legacy.agent(obs_dict)
    if obs_dict.get('select') is None and 'current' not in obs_dict:return base
    # Preserve the two validated tactical correction stages as one frozen executor.
    try:base=_rr.choose(obs_dict,base)
    except Exception:pass
    try:base=_td.choose(obs_dict,base)
    except Exception:pass
    return _ARBITER.choose(obs_dict,base)


def get_strategy_stats():
    return {'planner':dict(_PLANNER.stats),'macro_tree':_MT.get_stats(),'closeout':_CLOSE.get_stats(),'regret':_rr.get_stats(),'terminal':_td.get_stats()}

POLICY_RELEASE='v135_alakazam_safety_hardened'
