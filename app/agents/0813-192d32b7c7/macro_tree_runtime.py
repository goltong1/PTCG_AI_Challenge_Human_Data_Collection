...
class MacroTreePlanner:
    ...
    def reset(self):
        self.turn=-1;self.cached=TreeDecision();self.stats={'calls':0,'modes':{},'pv':{},'margins':[]}
    ...
    def _maxbase(self,q):
        ...
        return best
    def features(self,obs,arch:str):
        ...
        rock=sum(any(getattr(e,'id',None)==ROCK for e in (q.energies or [])) for q in luc)
    ...
    def _branches(self,s,arch):
        ...
        if s.get('opp_hand',0)>=6 and (s.get('hand_xero',0)>0 or s.get('hand_judge',0)>0) and (s.get('opp_stage2',0)>0 or s.get('_opp_bridge',0)>0 or s.get('opp_active_damage',0)>=120):
            n=self._advance(s)
    ...
    def decide(self,obs,arch:str,force:bool=False)->TreeDecision:
        turn=int(obs.current.turn or 0);self.stats['calls']+=1
        if not force and self.turn==turn:return self.cached
        s=self.features(obs,arch);base=self.value_model.value(s,arch)
        roots=[]
        for mode,n,bonus in self._branches(s,arch):
