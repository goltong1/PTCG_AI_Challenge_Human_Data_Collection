...
def _card_option_score(obs: Observation, option, mine, opponent, field: Counter, hand: Counter,
    ...
    if context in {SelectContext.SWITCH, SelectContext.TO_ACTIVE}:
        ...
        if cid == MORGREM:
            ...
            score += 6000
        if expendable_available and knockout_pressure and not can_stabilize_as_grimms:
    ...
    if context == SelectContext.ATTACH_FROM:
        ...
        if card.id == GRIMMSNARL_EX:
            return 27000 + deficit * 3500
        if card.id == MORGREM:
            return 24500 + deficit * 3000
        if card.id == IMPIDIMP:
...
def _forced_punk_up_count(obs: Observation, mine, field: Counter, plan: Plan, source: int) -> Optional[int]:
    ...
    wanted = max(2, min(5, marnie_deficit))
    ...
    if (plan.archetype == "crustle" or energy_punish) and reserve > 0:
        wanted = min(wanted, 2)
    return max(obs.select.minCount, min(obs.select.maxCount, dark_options, wanted, spendable if spendable > 0 else dark_options))
