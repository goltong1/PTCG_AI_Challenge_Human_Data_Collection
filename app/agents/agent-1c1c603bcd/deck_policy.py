    ...
    try:
        for idx in (bi,ci):
            ...
            try:
                ...
                value=_legacy._lt_state_value(policy.obs,st.observation)
    ...
    try:
        for idx,is_pivot in ((bi,False),(ci,True)):
            ...
            try:
                ...
                value=float(_legacy._lt_state_value(policy.obs,st.observation))
...
def agent(obs_dict:dict)->list[int]:
    ...
    if obs_dict.get('select') is None and obs_dict.get('current') is None:
        ...
        global _legacy
        _legacy=_load('lucario_frozen_v118','frozen_v118_tactical_executor.py')
        _PLANNER.reset();_MT.reset();_ARBITER.reset();_rr.reset();_td.reset();_HISTORY.reset();_HIST_GATE.reset();_HISTORY_REPLAY.reset();_LEAGUE_REPLAY.reset();_TEMPORAL_GRU.reset();_TEMPORAL_ATTENTION.reset();_MICRO_LLM.reset();_INTENT_LLM.reset();_QUALITY_LLM.reset();_HIER_STRATEGY_LLM.reset();_MISTAKE_ELIMINATOR.reset();_TEMPORAL_SAFETY.reset();_LOSS_REPAIR.reset();_TEXT_REASONER.reset();_SPECIAL_SITUATIONS.reset();_RECENT_HUMAN.reset();_V168_PRIZE.reset();_FINALDAY_PROOF.reset()
...
def get_strategy_stats():
    return {'planner':dict(_PLANNER.stats),'macro_tree':_MT.get_stats(),'closeout':_CLOSE.get_stats(),'regret':_rr.get_stats(),'terminal':_td.get_stats(),'history':_HISTORY.summary(),'history_gate':_HIST_GATE.get_stats(),'history_replay':_HISTORY_REPLAY.get_stats(),'league_replay':_LEAGUE_REPLAY.get_stats(),'temporal_gru':_TEMPORAL_GRU.get_stats(),'temporal_attention':_TEMPORAL_ATTENTION.get_stats(),'micro_llm':_MICRO_LLM.get_stats(),'intent_llm':_INTENT_LLM.get_stats(),'quality_llm':_QUALITY_LLM.get_stats(),'hier_strategy_llm':_HIER_STRATEGY_LLM.get_stats(),'mistake_eliminator':_MISTAKE_ELIMINATOR.get_stats(),'temporal_safety':_TEMPORAL_SAFETY.get_stats(),'loss_repair':_LOSS_REPAIR.get_stats(),'card_text_reasoner':_TEXT_REASONER.get_stats(),'recent_human':_RECENT_HUMAN.get_stats(),'special_situations':_SPECIAL_SITUATIONS.get_stats(),'v168_immediate_prize':_V168_PRIZE.get_stats(),'finalday_proof':_FINALDAY_PROOF.get_stats()}

POLICY_RELEASE='v171_finalday_mistakezero_basicfirst_luckaware_exactcf_proof'
