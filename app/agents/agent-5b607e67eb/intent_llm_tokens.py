...
def context_tokens(history,obs,family='unknown'):
    ...
    toks.extend(_strategic_tokens(history,obs,me,op,mine,enemy,turn))
    return enrich_context_tokens(toks)



def _strategic_tokens(history,obs,me,op,mine,enemy,turn):
    out=[]
    own_active=(mine.get('active') or []); own_bench=(mine.get('bench') or [])
    opp_active=(enemy.get('active') or []); opp_bench=(enemy.get('bench') or [])
    all_own=own_active+own_bench; hand=mine.get('hand') or []; hand_ids=[_cid(x) for x in hand]
    out += [f'STRAT_BENCH_FREE={max(0,min(5,_int(mine.get("benchMax"),5)-len(own_bench)))}',
            f'STRAT_OWN_BOARD={min(6,len(all_own))}',f'STRAT_OPP_BOARD={min(6,len(opp_active)+len(opp_bench))}']
    luc_ids={333,677,678}; util_ids={675,676}; dun_ids={305,66,306}
    mega=[c for c in all_own if _cid(c)==678]
    out += [f'STRAT_LUC_LINES={min(4,sum(1 for c in all_own if _cid(c) in luc_ids))}',
            f'STRAT_MEGA_COUNT={min(3,len(mega))}',f'STRAT_MEGA_EN1={min(3,sum(1 for c in mega if len(_energies(c))>=1))}',
            f'STRAT_MEGA_EN2={min(3,sum(1 for c in mega if len(_energies(c))>=2))}',
            f'STRAT_DUN_LINES={min(4,sum(1 for c in all_own if _cid(c) in dun_ids))}',
            f'STRAT_DUNEX_READY={min(2,sum(1 for c in all_own if _cid(c)==306 and len(_energies(c))>=3))}',
            f'STRAT_ACTIVE_LUC={int(bool(own_active and _cid(own_active[0]) in luc_ids))}',
            f'STRAT_ACTIVE_MEGA={int(bool(own_active and _cid(own_active[0])==678))}',
            f'STRAT_ACTIVE_UTIL={int(bool(own_active and _cid(own_active[0]) in (util_ids|dun_ids)))}']
    hc=Counter(hand_ids)
    for cid in (1225,1227,1213,1182,1142,1141,1159,1174,1197,1252,306,678,117):
        if hc.get(cid): out.append(f'STRAT_HAND_{cid}={min(3,hc[cid])}')
    out.append(f'STRAT_HAND_FENERGY={min(4,sum(1 for cid in hand_ids if cid in (6,20)))}')
    wall=draw=benchp=edenial=0
    for c in opp_active+opp_bench:
        cid=_cid(c)
        if not cid: continue
        st=' '.join(map(str,_sem.card_semantic_tokens(cid,'THREAT',True)))
        wall += int('PREVENT_DAMAGE' in st or 'PREVENT_FROM_' in st)
        draw += int('SKILLTAG=DRAW' in st or 'SKILLTAG=SEARCH' in st)
        benchp += int('BENCH' in st and ('DAMAGE' in st or 'COUNTER' in st))
        edenial += int('ENERGY' in st and ('DISCARD' in st or 'REMOVE' in st))
    out += [f'STRAT_OPP_WALLS={min(3,wall)}',f'STRAT_OPP_DRAW_ENGINES={min(4,draw)}',
            f'STRAT_OPP_BENCH_PRESSURE={min(3,benchp)}',f'STRAT_OPP_ENERGY_DENIAL={min(3,edenial)}']
    own_attach=[];own_attack=[];opp_attack=[]
    for e in list(getattr(history,'events',[]) or [])[-36:]:
        p=_int(e.get('playerIndex'),-1);cid=_int(e.get('cardId'));tid=_int(e.get('cardIdTarget'));aid=_int(e.get('attackId'))
        if p==me and tid and cid in (6,20): own_attach.append(tid)
        if p==me and aid: own_attack.append(aid)
        if p==op and aid: opp_attack.append(aid)
    if own_attach: out.append(f'STRAT_LAST_ATTACH_TARGET={own_attach[-1]}')
    if own_attack: out.append(f'STRAT_LAST_OWN_ATTACK={own_attack[-1]}')
    if opp_attack: out.append(f'STRAT_LAST_OPP_ATTACK={opp_attack[-1]}')
    my_pr=len(mine.get('prize') or []);op_pr=len(enemy.get('prize') or [])
    out += [f'STRAT_CLOSEOUT_SELF={int(my_pr<=2)}',f'STRAT_CLOSEOUT_OPP={int(op_pr<=2)}',
            f'STRAT_TEMPO={max(-3,min(3,op_pr-my_pr))}',f'STRAT_TURN={min(16,max(0,turn))}']
    return out
...
def enrich_context_tokens(tokens):
    ...
    if out and out[0]=='<BOS>':out.insert(1,'SEM_VERSION=160_INTENT6')
    else:out.insert(0,'SEM_VERSION=160_INTENT6')
    return out
...
def intent_context_tokens(tokens,max_len=72):
    """Small strategic-language stream for the intent Transformer."""
    src=enrich_context_tokens(tokens or [])
    head=[];strat=[];recent=[];board=[]
    keep_head=('FAMILY=','CTX=','TURN_BIN=','PHASE=','MY_PRIZE=','OPP_PRIZE=','PRIZE_DIFF=','MY_HAND=','OPP_HAND=','FLAG_','FIRST_REL=','HIST_')
    for t in src:
        s=str(t)
        if s=='<BOS>' or s.startswith(keep_head):head.append(s)
        elif s.startswith('STRAT_'):strat.append(s)
        elif s in ('<EVENT>','<DECISION>') or s.startswith(('EV_','DEC_')):recent.append(s)
        elif s.startswith(('OWN_ACTIVE_','OPP_ACTIVE_','SEM_CARD_SKILLTAG=','SEM_ATTACK_TAG=','STADIUM=')):board.append(s)
    out=['<BOS>']+[x for x in head if x!='<BOS>'][:18]
    out+=board[-14:]+recent[-22:]+strat[-30:]
    # de-duplicate only adjacent structural repeats; preserve causal recent events.
    if len(out)>max_len:out=[out[0]]+out[-(max_len-1):]
    return out
