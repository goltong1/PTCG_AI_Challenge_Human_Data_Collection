"""Hierarchical Mega Lucario submission policy.

The frozen v118 executor remains the tactical baseline.  History/temporal/LLM
residuals may propose supported alternatives, while the strategy, card-text,
terminal-proof and one-Prize guards remain authoritative.  Final live-match
cleanup is intentionally limited to proven invariants and does not add another
independent policy stack.
"""
from __future__ import annotations

import hashlib,importlib.util,os,sys
from dataclasses import dataclass
from cg.api import AreaType,CardType,EnergyType,OptionType,SelectContext,all_card_data,to_observation_class
...
_mlm_tok_mod=_load('lucario_micro_llm_tokens','micro_llm_tokens.py')
_intent_tok_mod=_load('lucario_intent_llm_tokens','intent_llm_tokens.py')
_mlm_mod=_load('lucario_micro_llm','micro_llm_runtime.py')
_intent_mod=_load('lucario_intent_llm','intent_llm_runtime.py')
_quality_mod=_load('lucario_quality_planformer','quality_planformer_runtime.py')
_hier_mod=_load('lucario_hier_strategy_moe','hier_strategy_moe_runtime.py')
_mistake_mod=_load('lucario_mistake_eliminator','mistake_eliminator_runtime.py')
_text_mod=_load('lucario_card_text_reasoner','card_text_reasoner_runtime.py')
_special_mod=_load('lucario_special_situations','special_situation_runtime.py')
_recent_mod=_load('lucario_recent_human_residual','recent_human_residual.py')
_v168_mod=_load('lucario_v168_immediate_prize','v168_immediate_prize_runtime.py')
_finalday_mod=_load('lucario_finalday_proof','final_day_proof_runtime.py')
_oneprize_mod=_load('lucario_oneprize_endgame','one_prize_endgame_runtime.py')
...
class C:
    ...
    MOUNTAIN=1252; SPIKEMUTH=1259; BLACK_BELT=1211; BASIC_F=6; ROCK_F=20
...
_MICRO_LLM=_mlm_mod.MicroDecoderPolicy(HERE,_HISTORY,_hr_mod,_mlm_tok_mod)
_INTENT_LLM=_intent_mod.IntentDecoderPolicy(HERE,_HISTORY,_hr_mod,_intent_tok_mod)
_QUALITY_LLM=_quality_mod.QualityPlanFormer(HERE,_HISTORY,_hr_mod,_intent_tok_mod)
_HIER_STRATEGY_LLM=_hier_mod.HierStrategyMoE(HERE,_HISTORY,_hr_mod,_intent_tok_mod)
_MISTAKE_ELIMINATOR=_mistake_mod.MistakeEliminator(HERE,_HISTORY,_hr_mod,_intent_tok_mod,_QUALITY_LLM)
_TEXT_REASONER=_text_mod.CardTextReasonerGate()
_SPECIAL_SITUATIONS=_special_mod.SpecialSituationGate()
_RECENT_HUMAN=_recent_mod.RecentHumanResidual(HERE)
_V168_PRIZE=_v168_mod.V168ImmediatePrizeGate()
_FINALDAY_PROOF=_finalday_mod.FinalDayProofGuard(HERE)
_ONEPRIZE_ENDGAME=_oneprize_mod.OnePrizeEndgameGuard()
...
SIG={
 ...
 'rocket':{C.TR_ARTICUNO,C.TR_MURKROW,C.TR_PORYGON,C.TR_PORYGON2,C.TR_PORYGONZ,C.TR_HONCHKROW},
 'starmie':{C.STARYU,C.STARMIE},
 'archaludon':{C.DURALUDON,C.ARCHALUDON,C.CINDERACE},
 # Froslass/Munkidori are splashable support cards and therefore do not identify
 # the archetype by themselves.  They modify the plan only after the dedicated
 # Marnie's evolution line has established the matchup.
...
class StrategyPlanner:
    ...
    def recognize(self):
        ...
        for a in ('crustle','dragapult','alakazam','rocket','starmie','archaludon','marnie','lucario','hydrapple','cynthia','slowking'):
    ...
    def build(self,obs):
        ...
        if a=='crustle' and ({C.DWEBBLE,C.CRUSTLE}&self.revealed):
            ...
            p.strategy='BYPASS_CRUSTLE';p.horizon=5
            # Dudunsparce ex is the default exact answer: Destructive Drill is a
            # text-proven 150-damage bypass that KOs Crustle exactly.  Do not flip
            # to Ogerpon merely because it happened to receive Energy first.
            p.primary=C.OGERPON if force_oger else C.DUDUN_EX
            p.secondary=C.DUDUN_EX if p.primary==C.OGERPON else C.OGERPON
            p.target_ids=(C.CRUSTLE,C.DWEBBLE)
            p.objective='keep Dudunsparce ex protected until three Energy, then lock Destructive Drill into Crustle'
            ...
            p.strategy='ONE_PRIZE_TEMPO';p.primary=C.LUCARIO;p.secondary=C.LUCARIO
            p.target_ids=(C.TR_PORYGON2,C.TR_PORYGON,C.TR_HONCHKROW,C.TR_MURKROW,C.TR_ARTICUNO)
            p.objective='farm one-Prize targets with minimum attack resources, preserve three-Prize Mega bodies, and never sink extra Energy into Dunsparce pivots'
        elif a=='dragapult':
            ...
            chip_support=bool(self.revealed & {C.FROSLASS,C.MUNKIDORI,C.SNORUNT})
            if C.GRIMMSNARL in self.revealed and chip_support:
                # Cornerstone blanks Shadow Bullet, but it still carries an Ability:
                # Freezing Shroud and Adrena-Brain therefore create a public counter
                # route that ignores the wall.  Preserve Ogerpon as a secondary pivot,
                # while the main prize plan becomes two Lucario waves plus removal of
                # the one-Prize chip engines.
                p.strategy='MARNIE_SUPPORT_DENIAL';p.primary=C.LUCARIO;p.secondary=C.OGERPON
                p.target_ids=(C.FROSLASS,C.MUNKIDORI,C.SNORUNT,C.MORGREM,C.IMPIDIMP,C.GRIMMSNARL)
                p.objective='remove Froslass and Munkidori chip engines while preserving two consecutive Lucario attackers'
            elif C.GRIMMSNARL in self.revealed:
                p.strategy='MARNIE_ABILITY_WALL';p.primary=C.OGERPON;p.secondary=C.LUCARIO
                p.target_ids=(C.GRIMMSNARL,C.MORGREM,C.IMPIDIMP)
                p.objective='lock isolated Punk Up attackers with Cornerstone while preserving a Lucario follow-up'
            else:
        ...
        # Persistence: exact plan identity is held for 2-5 turns.  Crustle primary is also
...
class StrategyArbiter:
    ...
    def choose(self,obs_dict,base):
        ...
        if (plan.strategy == 'HAND_DENIAL' or (plan.strategy in {'EVOLUTION_DENIAL','MARNIE_SUPPORT_DENIAL'} and plan.archetype == 'marnie')) and ctx in {SelectContext.SWITCH,SelectContext.TO_ACTIVE}:
            ...
            for i,o in enumerate(opts):
                ...
                if dmg>0 and q.hp<=dmg:
                    ...
                    if plan.archetype=='alakazam':
                        ...
                        if plan.strategy=='MARNIE_SUPPORT_DENIAL':
                            if q.id==C.FROSLASS:bonus=105
                            elif q.id==C.MUNKIDORI:bonus=95
                            elif q.id==C.SNORUNT:bonus=65
                            elif q.id==C.MORGREM:bonus=55
                            elif q.id==C.IMPIDIMP:bonus=35
                            elif q.id==C.GRIMMSNARL:bonus=30
                        else:
...
def _verify_micro_candidate(obs_dict:dict,base:list[int],candidate:list[int]):
    """Exact one-turn counterfactual check for a decoder proposal.

    The decoder performs semantic/history ranking; this verifier uses the CABT
    search API on one hidden-state determinization to reject proposals that lose
    immediate board value.  Only two branches are explored and at most once per
    game, so it remains far smaller than the disabled six-candidate rollout.
    """
    if len(base)!=1 or len(candidate)!=1:return False,{'reason':'shape'}
    bi,ci=int(base[0]),int(candidate[0])
    if bi==ci:return False,{'reason':'same'}
    obs=to_observation_class(obs_dict)
    if obs.select is None or obs.search_begin_input is None:return False,{'reason':'no_search'}
    if not (0<=bi<len(obs.select.option) and 0<=ci<len(obs.select.option)):return False,{'reason':'index'}
    policy=_legacy.LucarioPolicy(obs)
    yd,yp,od,opr,oh,oa=_legacy._lt_hidden_lists(policy)
    root=_legacy._search_begin(policy.obs,yd,yp,od,opr,oh,oa,manual_coin=False)
    root_turn=policy.state.turn;scores={};depths={}
    try:
        for idx in (bi,ci):
            sid=None
            try:
                st=_legacy._search_step(root.searchId,[idx]);sid=st.searchId;depth=0
                while (st.observation.select is not None and st.observation.current.result==-1
                       and st.observation.current.turn==root_turn and depth<12):
                    act=_legacy._lt_rollout_choice(st.observation)
                    mn=int(st.observation.select.minCount or 0);mx=int(st.observation.select.maxCount or 0)
                    act=[int(x) for x in (act or []) if 0<=int(x)<len(st.observation.select.option)]
                    # Search actions must satisfy the exact selection cardinality.
                    uniq=[]
                    for x in act:
                        if x not in uniq:uniq.append(x)
                    act=uniq[:mx]
                    if len(act)<mn:
                        for x in range(len(st.observation.select.option)):
                            if x not in act:act.append(x)
                            if len(act)>=mn:break
                    st=_legacy._search_step(st.searchId,act);sid=st.searchId;depth+=1
                value=_root_safe_branch_value(policy.obs,st.observation)
                value+=0.015*policy._score_option(policy.select.option[idx])
                scores[idx]=float(value);depths[idx]=depth
            finally:
                if sid is not None:
                    try:_legacy._search_release(sid)
                    except Exception:pass
    finally:
        try:_legacy._search_end()
        except Exception:pass
    if bi not in scores or ci not in scores:return False,{'reason':'branch_failed','scores':scores}
    gain=scores[ci]-scores[bi];need=float(_MICRO_LLM.gate.get('search_min_gain',1.0))
    return gain>=need,{'base_value':round(scores[bi],3),'candidate_value':round(scores[ci],3),'gain':round(gain,3),'min_gain':need,'depths':depths}


def _verify_mistake_luckaware(obs_dict:dict,base:list[int],candidate:list[int]):
    """Three paired CABT branches; promote only robust action quality, not one lucky rollout."""
    gains=[];samples=[]
    for _ in range(3):
        try:
            ok,meta=_verify_micro_candidate(obs_dict,base,candidate)
            g=float((meta or {}).get('gain',-1e9));gains.append(g);samples.append({'gain':round(g,3),'ok':bool(ok)})
        except Exception as e:
            gains.append(-1e9);samples.append({'error':repr(e)[:120]})
    finite=sorted(g for g in gains if g>-1e8)
    if len(finite)<3:return False,{'gain':-1e9,'reason':'insufficient_rollouts','samples':samples}
    med=finite[1];mean=sum(finite)/3.0;pos=sum(g>=25.0 for g in finite)
    # Final-day rule: all three paired branches must show material benefit; no lucky 2/3 promotion.
    accept=pos>=3 and med>=35.0 and mean>=25.0
    return accept,{'gain':round(med,3),'median_gain':round(med,3),'mean_gain':round(mean,3),'positive_rollouts':pos,'samples':samples,'luck_aware':True}

_MICRO_LLM.verifier=_verify_micro_candidate
_INTENT_LLM.verifier=_verify_micro_candidate
_QUALITY_LLM.verifier=_verify_micro_candidate
_HIER_STRATEGY_LLM.verifier=_verify_micro_candidate
_MISTAKE_ELIMINATOR.verifier=_verify_mistake_luckaware


_FINAL_CLEANUP_STATS={'empty_spikemuth_suppressed':0,'root_terminal_revalues':0}

def _root_safe_branch_value(root_obs,end_obs):
    """Evaluate terminal CABT branches from the original Lucario seat.

    CABT flips ``yourIndex`` when a rollout crosses to the opponent turn.  The
    legacy value function therefore can sign a finished game from the opponent's
    perspective.  Preserve its validated nonterminal heuristic, but make terminal
    win/loss values unambiguous from the root seat.
    """
    value=float(_legacy._lt_state_value(root_obs,end_obs))
    try:
        result=int(end_obs.current.result)
        if result in {0,1}:
            root_me=int(root_obs.current.yourIndex)
            _FINAL_CLEANUP_STATS['root_terminal_revalues']+=1
            return 1_000_000_000.0 if result==root_me else -1_000_000_000.0
    except Exception:
        pass
    return value

def _suppress_empty_spikemuth(obs_dict,out):
    """Drop the opponent's Spikemuth Gym no-op from our fixed non-Marnie deck.

    Live replay 93751150 repeatedly activated Spikemuth even though this deck has
    no Marnie's Pokémon.  We do not end the turn blindly; we remove only that
    dominated option and take the frozen executor's best remaining legal action.
    """
    if not (isinstance(out,list) and len(out)==1):return out
    try:obs=to_observation_class(obs_dict)
    except Exception:return out
    if obs.current is None or obs.select is None or obs.select.context!=SelectContext.MAIN:return out
    opts=list(obs.select.option or []);idx=int(out[0])
    if not (0<=idx<len(opts)):return out
    chosen=opts[idx]
    if chosen.type!=OptionType.ABILITY:return out
    c=_opt_card(obs,chosen,int(obs.current.yourIndex))
    if c is None or int(c.id)!=C.SPIKEMUTH:return out
    # This submission's fixed deck has no Marnie's Pokémon; verify the deck recipe
    # rather than assuming the fact from the opponent archetype.
    try:
        if any("Marnie's" in str(getattr(CARD.get(int(cid)),'name','')) for cid in _legacy.my_deck):return out
    except Exception:return out
    try:
        policy=_legacy.LucarioPolicy(obs)
        if policy.context==SelectContext.MAIN:policy._plan_attack()
        ranked=[]
        for i,o in enumerate(opts):
            if i==idx:continue
            if o.type==OptionType.ABILITY:
                oc=_opt_card(obs,o,int(obs.current.yourIndex))
                if oc is not None and int(oc.id)==C.SPIKEMUTH:continue
            ranked.append((float(policy._score_option(o)),-i,i))
        if ranked:
            ranked.sort(reverse=True);pick=ranked[0][2]
            _FINAL_CLEANUP_STATS['empty_spikemuth_suppressed']+=1
            return [pick]
    except Exception:
        return out
    return out

def _verify_finalday_pivot_once(obs_dict:dict,base:list[int],candidate:list[int],meta:dict):
    """One paired two-ply exact branch proof for a utility->Lucario pivot.

    Unlike the ordinary one-turn verifier, this follows the full public chain
    Energy->Air Balloon->Retreat->Mega Lucario->Aura Jab, then rolls one
    opponent turn.  This captures both Aura Jab acceleration and the risk of
    exposing a 3-Prize attacker before accepting the override.
    """
    if len(base)!=1 or len(candidate)!=1:return False,{'reason':'shape'}
    bi,ci=int(base[0]),int(candidate[0])
    obs=to_observation_class(obs_dict)
    ...
    want_active=int(meta.get('active_serial',-1));want_mega=int(meta.get('mega_serial',-1))
    scores={};depths={};summaries={}
    def _normalize(st,act):
        sel=st.observation.select;mn=int(sel.minCount or 0);mx=int(sel.maxCount or 0);n=len(sel.option or [])
        out=[]
        for x in act or []:
            try:x=int(x)
            except Exception:continue
            if 0<=x<n and x not in out:out.append(x)
        if len(out)<mn:
            for x in range(n):
        ...
        return out[:mx]
    try:
        for idx,is_pivot in ((bi,False),(ci,True)):
            sid=None
            try:
                st=_legacy._search_step(root.searchId,[idx]);sid=st.searchId;depth=0
                while (st.observation.select is not None and st.observation.current.result==-1
                       and st.observation.current.turn<=root_turn+1 and depth<70):
                    o=st.observation;sel=o.select;act=None
                    # Script only our original turn.  The opponent reply is rolled
                    # with the frozen executor so survivability is part of the proof.
                    if is_pivot and int(o.current.turn)==root_turn and int(o.current.yourIndex)==int(obs.current.yourIndex):
                        me=int(obs.current.yourIndex);mine=o.current.players[me]
                        if int(sel.context)==int(SelectContext.SWITCH):
                            for j,x in enumerate(sel.option or []):
                                try:q=mine.bench[x.index] if int(x.area)==int(AreaType.BENCH) else None
                                except Exception:q=None
                                if q is not None and int(q.serial)==want_mega:act=[j];break
                        if act is None and int(sel.context)==int(SelectContext.MAIN):
                            active=mine.active[0] if mine.active else None
                            if active is not None and int(active.serial)==want_active:
                                # Exit the utility Active.  Air Balloon is preferred
                                # only when Retreat is not yet legal.
                                for j,x in enumerate(sel.option or []):
                                    if x.type==OptionType.RETREAT:act=[j];break
                                if act is None:
                                    for j,x in enumerate(sel.option or []):
                                        if x.type!=OptionType.ATTACH:continue
                                        try:src=mine.hand[x.index] if int(x.area)==int(AreaType.HAND) else None
                                        except Exception:src=None
                                        if src is not None and int(src.id)==C.AIR_BALLOON and int(x.inPlayArea)==int(AreaType.ACTIVE):
                                            act=[j];break
                            elif active is not None and int(active.serial)==want_mega:
                                for j,x in enumerate(sel.option or []):
                                    if x.type==OptionType.ATTACK and int(getattr(x,'attackId',-1) or -1)==AURA_JAB:
                                        act=[j];break
                    if act is None:act=_legacy._lt_rollout_choice(o)
                    st=_legacy._search_step(st.searchId,_normalize(st,act));sid=st.searchId;depth+=1
                    if st.observation.current.turn>root_turn+1:break
                value=float(_root_safe_branch_value(policy.obs,st.observation))
                scores[idx]=value;depths[idx]=depth
                c=st.observation.current;me0=int(obs.current.yourIndex);mine0=c.players[me0];op0=c.players[1-me0]
                summaries[idx]={'turn':int(c.turn),'result':int(c.result),'my_prize':len(mine0.prize or []),'op_prize':len(op0.prize or []),
                                'active':[(int(q.id),int(q.hp),len(q.energies or [])) for q in (mine0.active or [])],
                                'bench':[(int(q.id),int(q.hp),len(q.energies or [])) for q in (mine0.bench or [])],
                                'opp_active':[(int(q.id),int(q.hp),len(q.energies or [])) for q in (op0.active or [])]}
            finally:
                if sid is not None:
                    try:_legacy._search_release(sid)
                    except Exception:pass
    ...
    if bi not in scores or ci not in scores:return False,{'reason':'branch_failed','scores':scores}
    gain=scores[ci]-scores[bi]
    # Require a material advantage after the opponent's reply; this is much more
    # conservative than the ordinary one-turn neural candidate threshold.
    return gain>=300.0,{'base_value':round(scores[bi],3),'candidate_value':round(scores[ci],3),'gain':round(gain,3),'depths':depths,'summaries':summaries}

def _verify_finalday_pivot(obs_dict:dict,base:list[int],candidate:list[int],meta:dict):
    """Luck-aware two-ply pivot verifier.

    A utility->Mega Lucario pivot is promoted only if three independent paired
    CABT branches all remain materially better after the opponent reply.
    This prevents one lucky rollout from exposing a 3-Prize attacker.
    """
    samples=[];gains=[]
    for _ in range(3):
        try:
            ok,d=_verify_finalday_pivot_once(obs_dict,base,candidate,meta)
            g=float((d or {}).get('gain',-1e9));gains.append(g);samples.append({'gain':round(g,3),'ok':bool(ok)})
        except Exception as e:
            gains.append(-1e9);samples.append({'error':repr(e)[:120]})
    finite=sorted(g for g in gains if g>-1e8)
    if len(finite)<3:return False,{'gain':-1e9,'reason':'insufficient_two_ply_rollouts','samples':samples}
    med=finite[1];mean=sum(finite)/3.0;pos=sum(g>=300.0 for g in finite)
    accept=pos>=3 and med>=300.0 and mean>=300.0
    return accept,{'gain':round(med,3),'median_gain':round(med,3),'mean_gain':round(mean,3),'positive_rollouts':pos,'samples':samples,'luck_aware_two_ply':True}

_FINALDAY_PROOF.verifier=_verify_finalday_pivot
_FINALDAY_PROOF.generic_verifier=_verify_mistake_luckaware
...
def agent(obs_dict:dict)->list[int]:
    ...
    if obs_dict.get('select') is None and obs_dict.get('current') is None:
        ...
        global _legacy
        _legacy=_load('lucario_frozen_v118','frozen_v118_tactical_executor.py')
        _FINAL_CLEANUP_STATS.update(empty_spikemuth_suppressed=0,root_terminal_revalues=0)
        _PLANNER.reset();_MT.reset();_ARBITER.reset();_rr.reset();_td.reset();_HISTORY.reset();_HIST_GATE.reset();_HISTORY_REPLAY.reset();_LEAGUE_REPLAY.reset();_MICRO_LLM.reset();_INTENT_LLM.reset();_QUALITY_LLM.reset();_HIER_STRATEGY_LLM.reset();_MISTAKE_ELIMINATOR.reset();_TEMPORAL_SAFETY.reset();_LOSS_REPAIR.reset();_TEXT_REASONER.reset();_SPECIAL_SITUATIONS.reset();_RECENT_HUMAN.reset();_V168_PRIZE.reset();_FINALDAY_PROOF.reset();_ONEPRIZE_ENDGAME.reset()
    base=_legacy.agent(obs_dict)
    if obs_dict.get('select') is None and obs_dict.get('current') is None:return base
    # Frozen tactical action is the only action the Transformer may restore;
    # the language model cannot invent a third unverified move.
    learned_anchor=list(base) if isinstance(base,list) else base
    ...
    # Disabled GRU and telemetry-only attention sidecars were removed in v180.
    # They had no action authority and no downstream consumer; the active ordered
    # history path continues through HistoryReplay and the strategy models below.
    # Dense 4:1 history Transformer compares the frozen tactical anchor with the
    # learned residual result.  Margin, independent support and exact CABT search
    # are all required before it can restore the anchor.
    try:base=_MICRO_LLM.veto(obs_dict,learned_anchor,base)
    except Exception:pass
    # v160 strategic-intent Transformer: terminal outcome remains only a coarse
    # macro prior.  v161 keeps its exact behavior but records the pre-intent action.
    pre_intent=list(base) if isinstance(base,list) else base
    try:base=_INTENT_LLM.choose(obs_dict,base)
    except Exception:pass
    # v161 counterfactual quality PlanFormer.  Its quality head is trained from
    # same-hidden-state CABT branch comparisons, not from blanket loss labels.
    # If the v160 stream already changed the move it may only apply a very strong
    # quality veto. Otherwise it may propose one same-type PLAY/ATTACH move, which
    # still requires local-quality support and the exact CABT verifier.
    try:
        if base!=pre_intent:base=_QUALITY_LLM.guard(obs_dict,pre_intent,base)
        else:base=_QUALITY_LLM.choose(obs_dict,base)
    except Exception:pass
    # v163 hierarchical temporal/card-strategy MoE.  A two-layer temporal encoder
    # routes every legal action through six strategy memories (Lucario chain,
    # Dudunsparce/Drill, resource, prize/attack, wall/text, recovery/denial), then
    # a candidate-arbiter Transformer proposes at most one cross-development move.
    # Human wins/losses are not blanket labels: support is built only from same-state
    # counterfactual quality and quality-weighted human actions.  Every proposal still
    # needs independent support, exact CABT branch gain, and the v162 quality veto.
    pre_hier=list(base) if isinstance(base,list) else base
    try:base=_HIER_STRATEGY_LLM.choose(obs_dict,base)
    except Exception:pass
    try:
        if base!=pre_hier:base=_QUALITY_LLM.guard(obs_dict,pre_hier,base)
    except Exception:pass
    # v170 mistake-elimination pass. Known replay mistakes are remembered only when
    # same-hidden-state counterfactual search proved the chosen action dominated.
    # Unseen states use the tuned Token-Cross critic to nominate alternatives; every
    # actual correction is re-proved by the current CABT exact branch verifier.
    try:base=_MISTAKE_ELIMINATOR.choose(obs_dict,base)
    except Exception:pass
    ...
    # Newest 37-game human/replay residual. It can intervene only when the
    # exact matchup/turn/action signature is repeatedly loss-dominant and a
    # legal alternative has independent winning-game support. Text invariants
    # and the special-situation gate below remain authoritative.
    try:out=_RECENT_HUMAN.choose(obs_dict,out,_PLANNER.plan)
    except Exception:pass
    # Final text-proven rule layer.  It reads the local CABT card/attack text at
    # runtime, so unseen IDs inherit wall/bypass/ability semantics without a
    # hard-coded matchup name.  Exact Dudunsparce-vs-Crustle Drill is immutable.
    try:out=_TEXT_REASONER.choose(obs_dict,out,_PLANNER.plan)
    except Exception:pass
    # Conservative terminal dominance and text-aware target handling.  This
    # may replace only a proven KO abandonment, a repeated Dunsparce Energy
    # overattachment, or an opponent target with an immediate Prize advantage.
    try:out=_SPECIAL_SITUATIONS.choose(obs_dict,out,_PLANNER.plan)
    except Exception:pass
    # Final-day basic-first invariants.  v168 is the exact same-turn prize chain;
    # the proof guard is the absolute last action authority and never requires
    # an opponent archetype label.
    try:out=_V168_PRIZE.choose(obs_dict,out)
    except Exception:pass
    try:out=_FINALDAY_PROOF.choose(obs_dict,out)
    except Exception:pass
    # v173 one-Prize/board-empty/shield invariant is the absolute last action authority.
    try:out=_ONEPRIZE_ENDGAME.choose(obs_dict,out)
    except Exception:pass
    # Final efficiency invariant: remove only the provably empty Spikemuth search
    # and continue with the best remaining validated tactical action.
    try:out=_suppress_empty_spikemuth(obs_dict,out)
    except Exception:pass
...
def get_strategy_stats():
    return {'planner':dict(_PLANNER.stats),'macro_tree':_MT.get_stats(),'closeout':_CLOSE.get_stats(),'regret':_rr.get_stats(),'terminal':_td.get_stats(),'history':_HISTORY.summary(),'history_gate':_HIST_GATE.get_stats(),'history_replay':_HISTORY_REPLAY.get_stats(),'league_replay':_LEAGUE_REPLAY.get_stats(),'micro_llm':_MICRO_LLM.get_stats(),'intent_llm':_INTENT_LLM.get_stats(),'quality_llm':_QUALITY_LLM.get_stats(),'hier_strategy_llm':_HIER_STRATEGY_LLM.get_stats(),'mistake_eliminator':_MISTAKE_ELIMINATOR.get_stats(),'temporal_safety':_TEMPORAL_SAFETY.get_stats(),'loss_repair':_LOSS_REPAIR.get_stats(),'card_text_reasoner':_TEXT_REASONER.get_stats(),'recent_human':_RECENT_HUMAN.get_stats(),'special_situations':_SPECIAL_SITUATIONS.get_stats(),'v168_immediate_prize':_V168_PRIZE.get_stats(),'finalday_proof':_FINALDAY_PROOF.get_stats(),'one_prize_endgame':_ONEPRIZE_ENDGAME.get_stats(),'final_cleanup':dict(_FINAL_CLEANUP_STATS)}

POLICY_RELEASE='v180_final_live_retrained_clean'
