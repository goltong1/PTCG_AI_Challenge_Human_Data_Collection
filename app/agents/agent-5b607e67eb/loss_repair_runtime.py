...
class ReplayLossRepairGate:
    ...
    def reset(self):
        ...
        self.pending_crustle_solrock_turn = -1
        self.crustle_solrock_attack_turn = -1
        self.pending_marnie_support_boss_turn = -1
        self.pending_marnie_support_attack_turn = -1
    ...
    def choose(self, obs_dict: dict, base: list[int], plan: Any = None) -> list[int]:
        ...
        # CROSS-PLAY PRIZE-DENIAL CHIP BRIDGE (Crustle/Ogerpon) -----------
        # Audited loss: both players had 2 Prizes left, Active Dudunsparce ex
        # sat at 130 HP versus a 210-HP Cornerstone Ogerpon, while an
        # Energy-ready Solrock + Lunatone pair was benched.  A raw 150 Drill
        # fails to KO and leaves a 2-Prize body exposed.  The one-Prize Solrock
        # can instead chip 70 with Cosmic Beam, setting an exact 150-HP Drill
        # finish while denying the immediate 2-Prize loss.  Keep this narrow to
        # the exact late prize-race geometry.
        if archetype == 'crustle' and op.active and op.active[0].id == OGERPON and 0 < len(mine.prize or []) <= 2 and 0 < len(op.prize or []) <= 2:
            if ctx in {SelectContext.SWITCH, SelectContext.TO_ACTIVE} and self.pending_crustle_solrock_turn == int(obs.current.turn or -1):
                for i,o in enumerate(opts):
                    q=_card(obs,o.area,o.index,getattr(o,'playerIndex',me))
                    if q is not None and q.id==SOLROCK and len(q.energies or [])>=1:
                        self.pending_crustle_solrock_turn=-1
                        self.crustle_solrock_attack_turn=int(obs.current.turn or -1)
                        return self._emit([i],'crustle:promote_solrock_chip_bridge')
            if ctx == SelectContext.MAIN and mine.active:
                if mine.active[0].id == SOLROCK and self.crustle_solrock_attack_turn == int(obs.current.turn or -1):
                    for i,o in enumerate(opts):
                        if o.type==OptionType.ATTACK and o.attackId==COSMIC:
                            self.crustle_solrock_attack_turn=-1
                            return self._emit([i],'crustle:cosmic_chip_before_drill')
                if mine.active[0].id == DUDUN_EX and int(mine.active[0].hp or 0) <= 150 and self._max_damage(mine.active[0],mine,op,op.active[0]) < int(op.active[0].hp or 0):
                    ready_sol=any(q.id==SOLROCK and len(q.energies or [])>=1 for q in (mine.bench or []) if q is not None)
                    lunar=any(q.id==LUNATONE for q in _field(mine))
                    retreat=next((i for i,o in enumerate(opts) if o.type==OptionType.RETREAT),None)
                    if ready_sol and lunar and retreat is not None:
                        self.pending_crustle_solrock_turn=int(obs.current.turn or -1)
                        return self._emit([retreat],'crustle:prize_denial_solrock_pivot')
        ...
        # Cross-play 2026-08-17: emergency Cornerstone pivot versus Marnie's
        # Grimmsnarl ex.  Punk Up is an Ability, therefore Cornerstone Stance
        # prevents Shadow Bullet's attack damage immediately even before Demolish
        # is charged.  Chip engines can still place/move counters, so this does
        # not replace support denial; it only prevents donating a fragile Mega
        # when the text-proven wall is already available.
        if archetype == 'marnie' and op.active:
            oger = next((q for q in _field(mine) if q.id == OGERPON), None)
            # Complete an audited Boss -> chip-engine KO sequence.  Boss target
            # selection also uses SWITCH/TO_ACTIVE contexts, so disambiguate by
            # playerIndex and only select opponent Munkidori/Froslass here.
            if ctx in {SelectContext.SWITCH, SelectContext.TO_ACTIVE} and self.pending_marnie_support_boss_turn == int(obs.current.turn or -1):
                cand=[]
                for i,o in enumerate(opts):
                    q=_card(obs,o.area,o.index,getattr(o,'playerIndex',me))
                    if q is None or getattr(o,'playerIndex',me)==me or q.id not in {MUNKIDORI,FROSLASS}:
                        continue
                    powered=1 if q.id==MUNKIDORI and len(q.energies or [])>=1 else 0
                    priority=(3 if powered else 2 if q.id==FROSLASS else 1)
                    cand.append((priority,-int(q.hp or 0),-i,i))
                if cand:
                    cand.sort(reverse=True)
                    self.pending_marnie_support_boss_turn=-1
                    self.pending_marnie_support_attack_turn=int(obs.current.turn or -1)
                    return self._emit([cand[0][-1]],'marnie_support:boss_chip_engine_target')

            # After Boss resolves, immediately take the support KO with Demolish
            # rather than spending more setup actions and reopening counter-play.
            if ctx == SelectContext.MAIN and self.pending_marnie_support_attack_turn == int(obs.current.turn or -1) and mine.active and mine.active[0].id==OGERPON and op.active and op.active[0].id in {MUNKIDORI,FROSLASS}:
                for i,o in enumerate(opts):
                    if o.type==OptionType.ATTACK and o.attackId==DEMOLISH:
                        self.pending_marnie_support_attack_turn=-1
                        return self._emit([i],'marnie_support:demolish_chip_engine')

            # The emergency wall logic below applies when Grimmsnarl is the
            # current attacker; when Boss has already moved a support Pokemon
            # Active, only the pending support-KO sequence above should fire.
            if not op.active or op.active[0].id != GRIMMSNARL:
                return base

            # Forced promotion: prefer the text-proven wall even at zero Energy.
            if ctx in {SelectContext.SWITCH, SelectContext.TO_ACTIVE}:
                for i, o in enumerate(opts):
                    q = _card(obs, o.area, o.index, getattr(o, 'playerIndex', me))
                    if q is not None and q.id == OGERPON:
                        return self._emit([i], 'marnie_emergency:promote_cornerstone')
            if ctx == SelectContext.MAIN and oger is not None:
                # If the wall is Active, invest manual Fighting Energy into the
                # 3-Energy Demolish route before generic Lucario overcharging.
                if mine.active and mine.active[0].serial == oger.serial:
                    # With a charged Cornerstone wall, Munkidori/Froslass are the
                    # real damage sources.  If Grimmsnarl is not an immediate KO,
                    # cash Boss into a guaranteed 140-damage support KO first.
                    if len(oger.energies or []) >= 3 and not obs.current.supporterPlayed:
                        current_dmg=self._max_damage(oger,mine,op,op.active[0])
                        live_support=[q for q in (op.bench or []) if q is not None and q.id in {MUNKIDORI,FROSLASS} and int(q.hp or 0) <= 140]
                        if live_support and current_dmg < int(op.active[0].hp or 0):
                            for i,o in enumerate(opts):
                                if o.type != OptionType.PLAY: continue
                                q=_card(obs,AreaType.HAND,o.index,me)
                                if q is not None and q.id==1182:
                                    self.pending_marnie_support_boss_turn=int(obs.current.turn or -1)
                                    return self._emit([i],'marnie_support:boss_chip_engine')
                    if len(oger.energies or []) < 3 and not obs.current.energyAttached:
                        cand=[]
                        for i,o in enumerate(opts):
                            if o.type != OptionType.ATTACH: continue
                            target=_card(obs,o.inPlayArea,o.inPlayIndex,me);energy=_card(obs,AreaType.HAND,o.index,me)
                            if target is not None and energy is not None and target.serial==oger.serial and energy.id in {BASIC_F,ROCK_F}:
                                cand.append((1 if energy.id==BASIC_F else 0,-i,i))
                        if cand:
                            cand.sort(reverse=True);return self._emit([cand[0][-1]],'marnie_emergency:charge_cornerstone')
                    # Do not voluntarily abandon a live Shadow-Bullet lock.
                    if _is_single(base):
                        try:
                            if opts[base[0]].type == OptionType.RETREAT:
                                end=next((i for i,o in enumerate(opts) if o.type==OptionType.END),None)
                                if end is not None:return self._emit([end],'marnie_emergency:hold_cornerstone')
                        except Exception: pass
                # If a damaged Mega is Active and cannot KO the current Grimmsnarl,
                # rotate into Cornerstone even when Demolish is not charged yet.
                elif mine.active and mine.active[0].id == LUCARIO and int(mine.active[0].hp or 0) <= 180:
                    dmg=self._max_damage(mine.active[0],mine,op,op.active[0])
                    if dmg < int(op.active[0].hp or 0):
                        retreat=next((i for i,o in enumerate(opts) if o.type==OptionType.RETREAT),None)
                        if retreat is not None:return self._emit([retreat],'marnie_emergency:retreat_to_cornerstone')
